...
- Current GC tuning note: the measured `aoc_day5` file-processing workload benefits substantially from `SPROUT_GC_THRESHOLD=4096` when run as a compiled native binary, but `aoc_day3` regresses badly at both `4096` and `8192`, so the runtime default has not been changed. Use `python3 -m sprout.cli compile examples/aoc_2025_day_5.sprout --native --with-stdlib -o /tmp/aoc_day5 && SPROUT_GC_THRESHOLD=4096 /tmp/aoc_day5 < day5input` as a workload-specific native override when that tradeoff is acceptable.
