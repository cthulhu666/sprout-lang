...
- Current GC tuning note: the measured `aoc_day5` file-processing workload benefits substantially from `SPROUT_GC_THRESHOLD=4096`, but `aoc_day3` regresses badly at both `4096` and `8192`, so the runtime default has not been changed. Use `SPROUT_GC_THRESHOLD=4096 mise exec -- just run-stdlib examples/aoc_2025_day_5.sprout < day5input` as a workload-specific override when that tradeoff is acceptable.
