...
def run_program(program: ast.Program, stdout: TextIO | None = None, argv: list[str] | None = None) -> int | None:
    ...
    def builtin_string_join_newlines(args: list[object]) -> object:
        parts: list[str] = []
        cur = args[0]
        while isinstance(cur, ADTValue):
            ctor = cur.constructor.rsplit(".", 1)[-1]
            if ctor == "Nil":
                break
            if ctor != "Cons" or len(cur.args) != 2:
                raise RuntimeError("string_join_newlines expects List String")
            item = cur.args[0]
            if not isinstance(item, str):
                raise RuntimeError("string_join_newlines expects List String")
            parts.append(item)
    ...
    env.set("string_join_newlines", BuiltinFunction(name="string_join_newlines", arity=1, fn=builtin_string_join_newlines))
