...
def symbol_metadata_in_source(module_source: str) -> list[SymbolMetadata]:
    composed = _compose_snapshot_source(module_source)
    with tempfile.TemporaryDirectory() as tmpdir:
        temp_path = (Path(tmpdir) / "repl_session.sprout").resolve()
        temp_path.write_text(composed, encoding="utf-8")
...
def python_snapshot_symbol_metadata_in_source(module_source: str) -> list[SymbolMetadata]:
    return symbol_metadata_in_source(module_source)


def structured_diagnostics_in_source(module_source: str) -> list[Diagnostic]:
    composed = _compose_snapshot_source(module_source)
    with tempfile.TemporaryDirectory() as tmpdir:
...
def diagnostics_in_source(module_source: str) -> list[tuple[str, int, int]]:
    diagnostics = structured_diagnostics_in_source(module_source)
