...
class Emitter:
    ...
    def string_const(self, value: str) -> tuple[str, int]:
        ...
        self.string_globals.append(f"{name} = private unnamed_addr constant [{len(data)} x i8] c\"{body}\"")
...
def _type_from_ast(node: ast.TypeExpr | None) -> LLType:
    ...
    if isinstance(node, ast.TypeApply):
        if (
            isinstance(node.base, ast.TypeName)
            and node.base.name == "IO"
            and isinstance(node.arg, ast.TypeName)
            and node.arg.name == "Unit"
        ):
            return I64
...
def _emit_call(
    ...
    if fn_name == "print":
        ...
        arg = _emit_expr(expr.args[0], locals_, globals_, sigs, emitter)
        if arg.typ == I64:
            tmp = emitter.tmp()
