...
bits  9–13  age    (saturating count of collections survived; 0 = allocated since the last sweep.
                    Written by the sweep's live branch, reset by any header (re)write — so an
                    in-place header edit that must preserve it masks rather than calls
                    sprout_hdr_write, as sprout_vec_release_inline_tail does)
bits 14–63  aux    (OBJ: (tag<<8)|arity; CSTR: byte length; CLOSURE: n_caps; TUPLE: word count;
                    VECTOR: inline element capacity; FREE/POISON: slot_bytes)
```
...
`SPROUT_VEC_INLINE_MAX` is 508 elements, and the slot there is *exactly* `SPROUT_LARGE_THRESHOLD` (8 + 24 + 508×8 = 4096), not under it — an ordinary slot only because `sprout_gc_alloc_block` takes the large path on `needed_slot > SPROUT_LARGE_THRESHOLD`. A `_Static_assert` beside the macro pins that, because normalising the comparison to `>=` would reroute every ceiling-sized vector through `region_table_insert` (O(live large regions)) with no test failing.

**A push out of the inline area shrinks the slot.** Growing an inline vector copies the elements to a `malloc` block, so the inline area is dead; `sprout_vec_release_inline_tail` therefore drops aux to 0 and writes a FREE header, with its slotmap bit, at the start of the vacated tail — making it a slot in its own right that the walk steps over and the sweep re-lists. Without it the tail would sit inside the live vector's slot for the rest of its life, up to 4064 bytes of it (`mutvec_new(n, v)` followed by a push is the reachable case). The tail is listed on its class freelist immediately, which is safe only because the sweep memsets and rebuilds the lists from the heap walk; it must not go through `fl_push_staged`, whose touched-class flags are only cleared inside a region walk.
...
**`SPROUT_GC_HDRCHECK=1` is the oracle for the slot walk**, and CI runs the whole suite under it. The walk's boundaries must agree with the slotmap exactly — a bit at every offset the walk stops on, and no bit strictly inside a step — plus a known kind byte, no slot past `bump`, a walk that ends exactly on `bump`, and (for a VECTOR) an aux that is non-zero exactly when `->data` points at the inline area. The two-sided slotmap check is what makes it bite: mutation-tested against four broken versions of the inline-tail release (a tail header sized 16 bytes too long, a missing slotmap bit, a freelist class one too high, a header never shrunk), it is the only mode that NAMES all four. The other columns, measured in the same run: the test's own assertions crash mid-suite on three of them and pass the fourth, `SPROUT_FL_VERIFY=1` adds nothing the plain run does not give, and `SPROUT_GC_STRESS=1` catches one — as a hang, not a failure, which is why `test-stress` now bounds each file. `tests/stdlib/test_gc_vec_buffer_heap_elems.spr` carries the table. A 16-byte desync in particular skips exactly one small slot, and then reads as a plausible run of CSTRs that ends neatly on `bump` — kind and bound checks alone see nothing. It costs +6% on an `emit-ir` run (docs/gates.md has the measurement), paid only where the flag is set — the interior scan is skipped for one-slot steps, which are most of the heap.
