...
- **Residuals — reachability assessment DONE (2026-07-29), both resolved.**
  - `TDict` expression: **not reachable — closed.** `lowering.lower_program` runs before `ast_to_ir` and eliminates every `TDict` (in-`TCall` → concrete instance fn-refs / hidden dict params; standalone → `TUnit`). `ast_to_ir`'s `"TDict not yet supported"` arm is structurally-unreachable dead code.
  - Refutable do-bind without `else` (`Just(x) <- e`): **was reachable — fixed.** It type-checked then crashed at codegen (no fallback, direct backend gone). The parser now rejects a refutable `<-` bind without `else` at parse time (`is_irrefutable_do_bind_pattern`), mirroring the sibling `let`-in-do rule; diagnostic "refutable `<-` binding in a do block requires an `else`". Prior-art (Rust/Swift/Scala-3) backed reject-at-parse, consistent with Sprout's `let..else`. Regression fixture under `tests/conformance/parse_error/`.
  - The M2/M3 codegen-parity axis is now closed; see `BACKLOG.md` (Model C item) for the M4 next step.
