...
3. Emit the new seed with the bridge binary — it compiles the new *source* from
   ...
   ```
   Verify: first line is `; Generated by sprout ir_lowering.sprout` (or whatever the
   current codegen header is), `grep -c '<old_builtin>' /tmp/seed.ll` == 0, and
...
unrooted (or under-rooted) heap value being swept mid-use. `just test` runs
only at the **default GC threshold**, where a
