...
The existing `docs/generational-gc-v1-draft.md` keeps per-object `ManagedNode` metadata (just splits the heap list) and explicitly notes its ceiling is 10–20%. That ceiling holds because per-alloc cost is unchanged.
