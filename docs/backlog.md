...
1. Push the current Sprout-hosted REPL toward a native-capable bridge.
   ...
   Completed groundwork: hidden `sprout analysis-service` bridge plus native-backed `repl_check_source(...)`, `repl_type_of_in_source(...)`, `repl_instances_in_source(...)`, `repl_eval_expr_in_source(...)`, and `repl_complete_in_state(...)` through that host-service path, with end-to-end native execution now covered by tests and exposed experimentally via `sprout.cli repl --native`, including launcher-side compiled-binary caching between launches and per-run analysis-service subprocess reuse.
