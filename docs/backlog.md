...
6. Continue native memory-management v1.
   ...
   Remaining v1 scope: close the remaining path-specific live-value gaps outside the current shadow-root coverage, decide whether the new threshold-triggered in-process collection remains debug-only or graduates into the normal native profile policy, and add stronger reclamation-focused validation.
