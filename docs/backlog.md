...
1. Push the current Sprout-hosted REPL toward a native-capable bridge.
   ...
   Completed groundwork: hidden `sprout analysis-service` bridge plus native-backed `repl_check_source(...)`, `repl_type_of_in_source(...)`, `repl_instances_in_source(...)`, `repl_eval_expr_in_source(...)`, and `repl_complete_in_state(...)` through that host-service path.
