...
| # | Scope | Deliverable |
|---|---|---|
| **M1** | DWARF in codegen; `--debug` flag; 4th IR section | `lldb ./myprog` supports `b myprog.spr:N`, `n`, `s`, `bt` at Sprout source granularity for user-module functions |
| **M2** | Extended `SproutCtorMeta` + pretty-printer tool | `p myval` at a breakpoint shows `Cons(42, Nil)` |
| **M3** | `just build-debug` / `just debug-run` recipes; README §Debugging section | One-command debug workflow documented end-to-end |
