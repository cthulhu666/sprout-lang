...
- [ ] `P2` **Instance selection keys on the head constructor alone, so disjoint parametric instances collide.** `instance C (List Int)` and `instance C (List Bool)` are rejected as "Overlapping instances for C" even though no type matches both. This is NOT a checker bug: `instance_key_and_class` deliberately mirrors `register_instance_marker`, and the dispatch table itself is head-granular (`@inst:C:List`), so accepting both would silently shadow one. Fixing it means widening the `@inst:` key to carry the full instantiated head — a dispatch-table representation change touching marker registration, lookup, lowering, and the seed, so it needs the Design Change Process, not a patch. Prior art: Haskell 2010 §4.3.2 forbids concrete instance arguments outright (`instance C [Int]` needs GHC's `FlexibleInstances`), so Sprout is already more permissive here than the standard and less than everyday GHC. Documented as a normative rule in spec §8.5. Found 2026-08-15 by the audit above.
...
- [ ] `P1` **Windows Milestone A — compile *to* Windows** (2026-08-15). Umbrella. Driver: **uncharted-suns is intended to ship on Steam**, which needs a Windows `.exe`; that game links this repo's `runtime/*.c` directly (its `Justfile:106`), so the game's Windows build is gated entirely on this runtime. Scope is the runtime and nothing else — **codegen is already portable**: `ir_lowering.sprout` emits `target triple = "unknown-unknown-unknown"` with no datalayout, and committed golden IR (including `musttail` and `llvm.stacksave`/`stackrestore`) cross-compiles to clean Win64 COFF objects at exit 0 for both `x86_64-pc-windows-gnu` and `aarch64-pc-windows-msvc` — reproducible with `clang --target=x86_64-pc-windows-gnu -c tests/golden/ir/examples__astar.sprout.ll -o /tmp/x.obj`. Toolchain decision: **develop with mingw-w64, ship with MSVC**, enforced by writing the Windows backend against pure Win32 + ISO C and never against mingw's POSIX shims (free if adopted at line one, expensive to retrofit). Rationale is risk, not Steamworks: Valve documents Windows support as MSVC-only, while `steam_api_flat.h`'s plain-C exports are reachable from any ABI. Minimum OS: **Windows 10 version 2004** (see W2). Milestone B — running the *compiler* on Windows (bash `justfile`/`scripts`, bootstrap seed, `mise`) — is explicitly out of scope; see its own entry below.
  - [ ] `P1` **W0 — toolchain seam + compile-only gate.** Add `runtime/sprout_context.h`, a 4-op seam (`ctx_adopt_current`/`ctx_create`/`ctx_switch`/`ctx_destroy`) wrapping the `ucontext` calls at `sprout_scheduler.c:399,446,509,549-553,616-621` and all three stack frees (`:451`, `:455`, `:743`). This is the one place an `#ifdef` in place will not do, because `ucontext_t` is embedded by value in `Task` and a fiber is an opaque `LPVOID`; everywhere else follows the existing precedent (9 `#ifdef __APPLE__` blocks in `sprout_runtime.c`, two backends in one `sprout_poll.c`). Add a `just windows-cross` recipe that *compiles* (not links) all three runtime TUs with `clang --target=x86_64-w64-mingw32` — runnable from macOS/Linux with no Windows machine, so it is the fast feedback loop for the whole port. Exit: all three `.c` files reach `clang -c` exit 0.
  ...
  - [ ] `P1` **W3 — Winsock, files, arena, threads, console.** Winsock2 (`SOCKET` is not an `int` fd; `WSAStartup`, `closesocket`, `WSAGetLastError`) across every `tcp_*` builtin and the handle table; `VirtualAlloc` `MEM_RESERVE`/`MEM_COMMIT` for the `mmap(PROT_NONE)` GC arena (a direct equivalent, the easiest item); `CreateThread` for the 2 `pthread_create` sites (async DNS; `stdatomic.h` is C11 and portable); `SetConsoleMode` + `ENABLE_VIRTUAL_TERMINAL_PROCESSING` for the 14 `termios`-family occurrences (the escapes `stdlib.terminal` emits work on Windows 10+); `GetModuleFileName` for `readlink`/`_NSGetExecutablePath`; `_ftelli64` at `sprout_runtime.c:7241`; stub `getrlimit`. Surfaces with no implementation return the established `"…unsupported on this platform"` shape (precedent `sprout_runtime.c:7676`) — never a silent success.
  - [ ] `P2` **W4 — crash diagnostics.** 10 `sigaction`/`sigaltstack` occurrences → a vectored exception handler; 13 `backtrace` occurrences → `CaptureStackBackTrace`/DbgHelp. May degrade to a loud stub without blocking a ship, but not to silence: the alternate-signal-stack design exists precisely so a stack-overflow SIGSEGV prints a diagnostic instead of dying as a bare exit-139.
