...
- [ ] `P0` End-to-end dynamic verification: `compile_driver_bin_stage1 --emit-ir` produces valid stage-2 IR for `compile_driver.sprout` without crashing or running out of memory. Stage-1 binary now built (1.52 MB; stage-0 IR emission: 749 MB peak RSS / 51s). Stage-2 self-compile was OOMing (exit 137, >4 GB) — `str_concat` GC leak now fixed (2026-05-08); stage-1 IR output was garbage integers due to generic-ctor field type erasure bug fixed (2026-05-09); verification pending after stage-0/1 rebuild.
...
- [ ] `P0` Add `BootstrapStage3Tests` that runs stage-1 → stage-2 IR → stage-2 binary → stage-3 IR and asserts byte-equality (or structural equality) between stage-2 and stage-3 IR (the fixed-point check). Unblocked by `str_concat` GC fix (2026-05-08); depends on stage-2 self-compile completing successfully.
