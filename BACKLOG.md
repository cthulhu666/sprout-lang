...
- [ ] `P2` **Enforce the effect rules (spec-v0.md §7 rules 8, 9, 11).** The measurement is done and the inference now produces a signal; enforcement itself is not started. **Read `docs/effect-enforcement-v0.md` first** — it has the full break list, the two instrument limitations, and the numbers. Summary of where this stands:
  ...
  - **BLOCKED ON: is `panic` an effect?** `prelude.sprout:1288` declares it `!{IO}`, which is what produces 9 of the 12. It diverges rather than returning, so annotating every pure function with an internal-error fallback is arguably a false statement as well as a burden. Options are pure / its own effect / leave it. Wants a verified prior-art survey per the Design Change Process; that survey has **not** been done.
