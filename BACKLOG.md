...
- [ ] `P2` **Enforce the effect rules (spec-v0.md §7 rules 8, 9, 11).** The measurement is done and the inference now produces a signal; enforcement itself is not started. **Read `docs/effect-enforcement-v0.md` first** — it has the full break list, the two instrument limitations, and the numbers. Summary of where this stands:
  ...
  - **Enforcement is now unblocked and is the only thing left.** Flip `check_fn_body`'s recorded pair (and `check_instance_method`'s) into a `BodyErr` on the definite case — declared pure, inferred `!{IO}` — and accept everything else, including an unresolved effect variable, since the rule is subsumption and the checker must accept when unsure. **Note the flip changes what `tests/effects/canaries.spr` IS:** it currently holds seven deliberately-broken declarations and would stop compiling, so the must-be-flagged cases have to become `tests/conformance/type_error/` fixtures (one per propagation path — `shout`, `calls_loud`, `each_lambda`, `each_named`, `apply_now`, `via_local`, `describe(Noisy)`) while the must-not-be-flagged cases stay behind as the file that must still compile. Budget for that restructure, not just the two-line comparison.
  - **Also still open:** writing an inferred effect back to the env so the report becomes a fixed point rather than one wave; and the conformance fixture for `fn shout(s: String) -> Unit = print(s)`, which cannot land before the implementation because `tests/conformance/type_error/` has no `XFAIL` manifest.
  ...
  - **Minor, still open:** `infer.call_effect_of` with `argc <= 0` returns `scheme_effects(scheme)` raw — the program-level `EffectVar("e")` rather than a freshened `$eN`. It is now the only unfreshened effect that can escape into a reported value. The census shows no damage (a nullary `!{e}` has no argument to bind it from), so this is a tidiness item, not a correctness one.
