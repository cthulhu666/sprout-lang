...
- [ ] `P0` Add `BootstrapStage3Tests` that runs stage-1 → stage-2 IR → stage-2 binary → stage-3 IR and asserts byte-equality (or structural equality) between stage-2 and stage-3 IR (the fixed-point check). Unblocked by `str_concat` GC fix (2026-05-08) and stage-2 SIGABRT fix (2026-05-14); `just build-stage2` now succeeds cleanly.
