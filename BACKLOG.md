...
- [ ] `P2` **Enforce the effect rules (spec-v0.md §7 rules 8, 9, 11).** The measurement is done and the inference now produces a signal; enforcement itself is not started. **Read `docs/effect-enforcement-v0.md` first** — it has the full break list, the two instrument limitations, and the numbers. Summary of where this stands:
  ...
  - **The one thing now blocking enforcement: `infer_lambda`'s attribution.** It reports the lambda *body's* effect as the enclosing expression's, so a pure function that builds an IO-performing closure reads as IO. All **six** real corpus gaps are this and nothing else (`log.stderr_logger`, `http_middleware.with_logging`, `http_web_server.routes`, `fixture_b1_partial.get_at`, `capture_logger` ×2) — enforcing today would reject six correct programs. Correcting it is only *now* safe: the over-approximation used to be the sole path by which an effect crossed a higher-order boundary, and unification has replaced it. See `docs/effect-enforcement-v0.md` §9.4.
