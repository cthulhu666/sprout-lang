...
- [~] `P0` Bootstrap pipeline parity for `--emit-ir codegen.sprout` (prerequisite for stage-1 / stage-3 fixed point). **Three blockers fixed (2026-05-03):** `string_concat_many` seeded in bootstrap checker env + codegen FN_SIG (`b81d925`); `lookup_type_var` regression repaired so qualified annotation types normalize to bare names (`4f12599`); unifier strips module prefixes symmetrically on TConst-vs-TConst comparison (`4199150`). With all three landed, stage-0 binary now passes `bundle → check → lower` cleanly on `codegen.sprout`. **One blocker remaining:** `--emit-ir` fails at `check2` (post-lowering typecheck) with `Type mismatch: String vs t_N -> String in function stdlib.compiler.codegen.emit_str_ptr`. Reproduces *only* via `--emit-ir`, not via plain typecheck (the bug is in the lowering output). Suspected cause: typeclass dispatch on `to_string(int_to_string(len))` produced by the template-interp desugar — lowering picks the wrong instance for the wrapped `to_string` call, treating the argument's type as a function shape rather than the call's String result. The Python pipeline (`sprout/typeclass_lowering.py`) handles the same construct correctly. **Next-session starting point:** trace `to_string(int_to_string(len))` through `stdlib/compiler/lowering.sprout`'s `lower_expr` and `expand_call_args` (line ~480), comparing TDict witness selection against Python's `typeclass_lowering.py` for the same call shape. Minimal repro: `./compile_driver_bin --emit-ir "$(pwd)/stdlib" stdlib/compiler/codegen.sprout`. Note that minimal isolated test files (e.g. `\`value: ${int_to_string(n)}\`` in a fresh module) do *not* reproduce — the bug appears to require the full module-import context, suggesting it surfaces only when bundler-qualified names interact with class-method dispatch.
