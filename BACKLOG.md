...
- [ ] `P0` Investigate the stage-1-vs-stage-0 memory ratio. Likely sources: per-let GC-root push overhead from the M7 TDoLetStep fix (could batch consecutive lets into a single push), Sprout codegen's lack of stack-slot promotion vs. Python codegen's, and possible inefficiencies in the AST/typed-AST representation that surface at scale. Baseline numbers needed before declaring M7 done.
