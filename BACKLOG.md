...
- [ ] `P2` **Enforce the effect rules (spec-v0.md §7 rules 8, 9, 11).** The measurement is done and the inference now produces a signal; enforcement itself is not started. **Read `docs/effect-enforcement-v0.md` first** — it has the full break list, the two instrument limitations, and the numbers. Summary of where this stands:
  ...
  - **Is `panic` an effect? — surveyed 2026-08-16, recommendation: NO, declare it pure.** `prelude.sprout:1288` declares it `!{IO}`, which is what produces 9 of the 12. Six languages verified against primary sources (Koka, Haskell, Rust, Java, Swift, Zig): **none folds abort into its I/O channel.** Koka is the only one that tracks it at all, as a separate weaker effect *inside* its `pure` alias (`alias pure = <exn, div>`). Java/Swift/Zig each track recoverable failure in the signature and deliberately exempt the abort — despite every one of those aborts also printing a diagnostic, which is the one real counter-argument here (`panic` → `tcp_fail` → `fprintf(stderr, …)`, `sprout_runtime.c:5037`). It does not survive because an effect matters when a *continuation* can observe it, and `panic` has none. Full survey with sources: `docs/effect-enforcement-v0.md` §6. **Needs a call from the user**, and carries a normative edit: spec §6's literal rule ("`!{IO}` when the call may interact with … terminal IO") says otherwise. With `panic` pure the 12 gaps become 3, all of them the known lambda over-approximation, so rule 8 would cost **zero** source annotations.
  - **Also still open:** effect-variable solving (`!{e}` is written in `list_each`/`list_fold` in the prelude, never quantified, never bound — enforcement cannot ship Pure/IO-only); writing an inferred effect back to the env so the report becomes a fixed point rather than one wave; and the conformance fixture for `fn shout(s: String) -> Unit = print(s)`, which cannot land before the implementation because `tests/conformance/type_error/` has no `XFAIL` manifest.
