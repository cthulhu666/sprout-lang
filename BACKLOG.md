...
- [ ] `P3` **A malformed `let … in` do step reports the pre-fix `Expected pattern` error (2026-08-14).** `parse_do_let_in_expr` discards the expression parse's `Err` and returns `Nothing`, so a step like `let x = 1` / `in x +` falls through to the statement path, takes the multi-binding branch and dies at the `in` with `Expected pattern` — the exact misleading message the `let … in`-as-do-step fix was written to eliminate, resurfacing whenever the body itself is broken. Confirmed. Fix: when the expression parse actually reached the slice's depth-0 `in`, surface its error instead of falling back. Care needed — the fallback must still win for a genuine statement whose right-hand side merely *contains* a nested `let … in`, which is the case `tests/stdlib/test_do_let_in_step.spr` pins.
