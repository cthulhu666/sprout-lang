...
- [ ] `P1` **A `Double` does not survive `stringify` → `json_parse`, and above `1e17` does not survive at all (reported 2026-08-20 from `uncharted-suns`'s `game.wire_json`; independently reproduced here on `c90bf97a`).** The writer and the reader are different implementations with different capabilities: `json_stringify` is C (`runtime/sprout_runtime.c:6231`, `snprintf("%.17g")`), while `json_parse` reads numbers with the pure-Sprout `parse_double` (`stdlib/json.sprout:112` → `stdlib/prelude.sprout:1452`). The writer emits three things the reader cannot read back. This is **not only a self-round-trip defect** — `1e+17` is a valid JSON number and `%g`-style exponents are what other producers emit, so `json_parse` also rejects standards-conformant JSON from any non-Sprout source. Consumer impact is recorded in `uncharted-suns` `docs/net-stage1-v0.md §5`, where the game gave up on JSON numbers entirely and now ships every `Double` as `JsonInt(double_to_bits(x))`. Sub-defects, each measured, in priority order:
  ...
  Repro for (a)–(c) and the (d) harness: the four probe programs used to measure this are small and self-contained; regenerate them from the measurements above, or add them as `tests/stdlib/` cases with the fix (per Definition of Ready #3, each defect needs a regression test that fails first). Fixing (a) and (c) is a `stdlib/prelude.sprout` change — non-compiler stdlib, so `seed-fp-ack` rather than a full reseed, but it does need the full `just test` gate (language-semantics surface).
