...
- [ ] `P2` `stdlib.terminal` is invisible in the REPL now that it declares types. Pre-existing compiler defect, newly applicable: `docs/repl-env-type-vocabulary-v0.md` §1.2 measured eleven top-level stdlib modules whose `type` declarations fail to load in the REPL (`unknown type name \`Vec\`/\`Dict\`/\`Result\` in declaration …`), and M0 added `TermSize`/`TermInput` to `stdlib/terminal.sprout`, which had no `type` before and so was not on that list. Nothing in the M0 change causes it and compiled programs are unaffected — it is the module-loader's type vocabulary, fixed once for all twelve modules or not at all.
