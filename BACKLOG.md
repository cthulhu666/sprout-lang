...
- [~] `P1` **A `Double` does not survive `stringify` → `json_parse`, and above `1e17` does not survive at all (reported 2026-08-20 from `uncharted-suns`'s `game.wire_json`; independently reproduced here on `c90bf97a`). SUB-DEFECTS (a), (b), (c) AND (e) FIXED 2026-08-20; only (d) sub-ULP accuracy remains, an accuracy limit rather than a plain defect.** The writer and the reader are different implementations with different capabilities: `json_stringify` is C (`runtime/sprout_runtime.c:6231`, `snprintf("%.17g")`), while `json_parse` reads numbers with the pure-Sprout `parse_double` (`stdlib/json.sprout:112` → `stdlib/prelude.sprout:1452`). The writer emits three things the reader cannot read back. This is **not only a self-round-trip defect** — `1e+17` is a valid JSON number and `%g`-style exponents are what other producers emit, so `json_parse` also rejects standards-conformant JSON from any non-Sprout source. Consumer impact is recorded in `uncharted-suns` `docs/net-stage1-v0.md §5`, where the game gave up on JSON numbers entirely and now ships every `Double` as `JsonInt(double_to_bits(x))`. Sub-defects, each measured, in priority order:
