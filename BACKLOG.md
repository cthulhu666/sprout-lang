...
- [ ] `P2` **An ambiguous class-method chain inside a top-level `let` reaches the LINKER, unlocated (found 2026-08-20 probing the ambiguity guard's boundary).** `let s = to_str(from_int(7))` with two `Codec` instances — the `show . read` ambiguity, where the intermediate type occurs only between the two calls — compiles to a call with no dictionary and fails as `use of undefined value '@from_int'`, with no Sprout source position. The identical shape inside a `fn` body IS rejected at check time: `tests/conformance/type_error/ambiguous_class_tyvar` pins that, and its header records the linker failure as the *pre-fix* behaviour. So the rejection lives somewhere a top-level `let` does not reach — the same family as the `resolve_dispatch_typed_expr` gap fixed above, and worth checking whether it is literally the same call-site list (`check_fn_body` / `check_instance_methods`). The `ambiguous typeclass binding` guard added alongside that fix does NOT cover this and is not meant to: the binding's own type is ground (`String`), so it quantifies nothing, and the undetermined variable is internal to the initializer. Pre-existing, verified identical on the pre-change compiler. Acceptance: the top-level form is rejected with the same located diagnostic as the `fn`-body form; extend the conformance fixture with the `let` variant rather than adding a separate one.
