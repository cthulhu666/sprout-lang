...
- [ ] `P3` **Decide whether to retire the brace form of `class`/`instance` bodies (2026-08-16).** The layout form landed as the idiomatic spelling and the whole corpus was migrated to it, but `parse_class_body`/`parse_instance_body` still accept `{ … }`, chosen by one token of lookahead. Keeping both was deliberate: it let the committed seed keep parsing the still-braced sources while they were swept, so the migration was an ordinary commit rather than a 2-step bootstrap. That reason has now expired — no source in the tree uses braces. Prior art splits: Haskell keeps both permanently (§2.7 layout is defined as brace insertion, and the two "can be freely mixed"), Scala 3 likewise keeps optional braces; PureScript documents layout only. Argument for removal: one spelling to learn, and the spec no longer has to describe two forms. Argument against: the brace form is the escape hatch for anything layout cannot express, and nothing costs us by keeping it. Wanted either way is a decision recorded in the spec, not a silent drift — if it stays, say so normatively; if it goes, it is a small parser deletion plus the §8.5 "Declaration syntax" rewrite.
