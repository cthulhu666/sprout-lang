...
- [ ] `P2` **`merge_effects` drops one variable side (`infer.sprout` ~340-346)** (fundamentals review, static finding — masked today by effect enforcement being off, cf. D2/W6 below). When merging two effect rows where at least one side is a variable, `merge_effects` only propagates one of the two variable sides instead of unifying/merging both — a latent effect-inference bug currently invisible because the effect system isn't enforced (a pure function calling `!{IO}` code passes the checker regardless, per the fundamentals review's item 1). Revisit once the deferred effect-system design pass (D2/W6) lands and effects actually gate compilation — fixing this before enforcement exists has no observable payoff and no test would catch a regression today. Full findings: `docs/fundamentals-code-review-handoff-2026-07-03.md`.
  - **MEASURED 2026-08-15 — "wire up `unify_effects`" is not the shape of this work, and the inference produces no signal to wire.** Instrumented the `ast.FnDecl` arm of `infer.sprout` (the site that binds `InferOk _ _ _ _` and discards the body effect), built a stage-2, and dumped declared-vs-inferred for every function declaration in one real compile. **316 declarations: 289 `pure`→`pure`, 16 `!{e}`→`pure`, 11 `!{IO}`→`pure`. Zero inferred `IO`, ever** — including `main`, whose body is literally `print(...)`. Three independent breaks, all of which must be fixed before any fallout can even be counted:
    1. **The declared effect never reaches the type.** `scheme_from_fn_parts_inner` puts it in `Scheme`'s effect field but builds the type with `build_fn_type_modes`, which **hardcodes `types.EffectPure` on every arrow** (`infer.sprout:352`). The arrow says pure while the Scheme says `!{IO}`. Fixing this is a design decision, not a typo: for a curried `fn f(a, b) -> C !{IO}`, which arrow carries the effect? The innermost (effect at saturation) is the conventional answer, and partial application makes the choice observable.
    2. **`Scheme`'s effect field is write-only as far as inference is concerned.** `types.scheme_effects` has exactly four readers repo-wide, all in `types.sprout`: one free-variable computation and two printers. Inference never reads it, so `callee_eff` at every call site is pure and `merge_effects` only ever merges pure with pure. **The consumer side is the part that is already done** — `merge_effects` is correctly threaded through calls, args, `if`, match arms and do-steps. It is the *source* that is dead.
    3. **`unify_effects` would accept everything if wired in unchanged.** `unifier.sprout:341-342` returns `Ok` for both `(EffectPure, EffectIO)` and `(EffectIO, EffectPure)` — deliberately, per its "Conservative: IO and Pure unify as IO" comment. Enforcement needs a *subsumption* check (inferred ⊑ declared), not unification; calling the existing function at the FnDecl site is a literal no-op.
  - Note the spec's §7 enforcement note (added by the entry below) says effects are "carried on `TFunc`". That is inaccurate in a way that matters to whoever picks this up: they are carried on `Scheme`, and the `TFunc` arrows are pure. "Never unified" also understates it — they are never *produced*.
  - **Consequence for sequencing:** the fallout of turning enforcement on is currently **unmeasurable**, because no body infers an effect. Expect it to be large — every `!{IO}` in `stdlib/` and `stdlib/compiler/` is presently unverified — and expect effect-variable subsumption (`!{e}`, 16 of the 316) to need its own rules. A ready-made conformance fixture for the first case enforcement must reject (`fn shout(s: String) -> Unit = print(s)`) belongs under `tests/conformance/type_error/`; it cannot land before the implementation because `type_error` has no `XFAIL` manifest (only `tests/conformance/run/` does).
...
- [ ] `P1` **Windows Milestone A — compile *to* Windows** (2026-08-15). Umbrella. Driver: **uncharted-suns is intended to ship on Steam**, which needs a Windows `.exe`; that game links this repo's `runtime/*.c` directly (its `Justfile:106`), so the game's Windows build is gated entirely on this runtime. Scope is the runtime and nothing else — **codegen is already portable**: `ir_lowering.sprout` emits `target triple = "unknown-unknown-unknown"` with no datalayout, and committed golden IR (including `musttail` and `llvm.stacksave`/`stackrestore`) cross-compiles to clean Win64 COFF objects at exit 0 for both `x86_64-pc-windows-gnu` and `aarch64-pc-windows-msvc` — reproducible with `clang --target=x86_64-pc-windows-gnu -c tests/golden/ir/examples__astar.sprout.ll -o /tmp/x.obj`. Toolchain decision: **develop with mingw-w64, ship with MSVC**, enforced by writing the Windows backend against pure Win32 + ISO C and never against mingw's POSIX shims (free if adopted at line one, expensive to retrofit). Rationale is risk, not Steamworks: Valve documents Windows support as MSVC-only, while `steam_api_flat.h`'s plain-C exports are reachable from any ABI. Minimum OS: **Windows 10 version 2004** (see W2). Milestone B — running the *compiler* on Windows (bash `justfile`/`scripts`, bootstrap seed, `mise`) — is explicitly out of scope; see its own entry below.
  ...
  - [ ] `P1` **W0b — the `sprout_context.h` seam.** Add `runtime/sprout_context.h`, a 4-op seam (`ctx_adopt_current`/`ctx_create`/`ctx_switch`/`ctx_destroy`) wrapping the `ucontext` calls at `sprout_scheduler.c:399,446,509,549-553,616-621` and all three stack frees (`:451`, `:455`, `:743`). This is the one place an `#ifdef` in place will not do, because `ucontext_t` is embedded by value in `Task` and a fiber is an opaque `LPVOID`; everywhere else follows the existing precedent (9 `#ifdef __APPLE__` blocks in `sprout_runtime.c`, two backends in one `sprout_poll.c`). **Deliberately contains no Windows code**: it is a pure POSIX-side refactor of the most delicate file in the runtime, so the gates that already exist can prove it correct — `just test`, `just linux-smoke` (epoll/timerfd backend) and the example canary. Extracting the seam while it still has one implementation is the cheap ordering; doing it after W1 means debugging the refactor and the fiber port at the same time, on a platform that cannot yet run the suite. Exit: those POSIX gates still pass, with no behavioural change.
  - [ ] `P1` **W1 — green threads: `ucontext` → Win32 Fibers.** The architectural item. `ConvertThreadToFiber` converts the *calling thread*, i.e. task-0/main, whose "native stack, never freed" special case (`g_task0.stack = NULL`, `sprout_scheduler.c:519`) maps exactly onto a converted-thread fiber; the pump (`:549-553`) and every spawned task (`:616-621`) become `CreateFiber` fibers; switching is `SwitchToFiber`. **The risk is `force_drop_task`** (`:678`), which frees a parked task's stack without unwinding (`:743`) and becomes `DeleteFiber`. Microsoft documents exactly two dangerous cases — deleting the *currently running* fiber (its thread calls `ExitThread`), and deleting another thread's *selected* fiber — and Sprout is in neither: the scheduler is single-threaded and force-drops run from the pump against a parked task. Two constraints to preserve: a fiber function that returns exits its thread (`task_trampoline` already never returns, `:509` — keep it that way), and plain TLS does *not* switch with the fiber (the runtime uses no TLS today; any added TLS must become FLS).
  - [ ] `P1` **W2 — Windows poller backend (`WSAPoll` first).** A third backend behind the existing 6-function interface (`sprout_scheduler.h:71-100`). `WSAPoll` is the only candidate needing neither a completion↔readiness translation nor undocumented kernel interfaces: `POLLIN` = `POLLRDNORM|POLLRDBAND`, `POLLOUT` = `POLLWRNORM`, error conditions are always reported unrequested, and negative `fd` entries are ignored with `POLLNVAL` — which gives `sprout_poll_remove`'s idempotent contract for free. **Two verified caveats.** (1) Microsoft: *"As of Windows 10 version 2004, when a TCP socket fails to connect, (POLLHUP | POLLERR | POLLWRNORM) is indicated"* — before that a failed `connect()` went unreported, which would break `tcp_connect`'s park; this is what sets the OS floor. (2) A blocking `WSAPoll` is an alertable wait; Sprout queues no APCs, so it is a do-not-start note. Timers: no timerfd equivalent, so keep a deadline min-heap and pass the nearest deadline as the poll timeout — permitted by the token-based `sprout_poll_add_timer` contract. Consequence to document rather than leave undefined: the heap has no descriptor pressure, so the "could not arm, returns 0" path (`sprout_scheduler.h:77-89`, a real failure under `ulimit -n` on the epoll backend) is **unreachable** on Windows. Fallbacks only if `WSAPoll` is measured inadequate: an AFD/wepoll-style shim (what Rust's `mio` does — but undocumented NT interfaces *and* vendored third-party code, so it needs explicit approval) or IOCP (what Go and libuv use — but a completion model, reshaping the park protocol rather than just the backend).
  - [ ] `P1` **W3 — Winsock, files, arena, threads, console.** Winsock2 (`SOCKET` is not an `int` fd; `WSAStartup`, `closesocket`, `WSAGetLastError`) across every `tcp_*` builtin and the handle table; `VirtualAlloc` `MEM_RESERVE`/`MEM_COMMIT` for the `mmap(PROT_NONE)` GC arena (a direct equivalent, the easiest item); **no work for the 2 async-DNS `pthread_create` sites under mingw** (W0a measured `pthread.h` + winpthreads as present; `stdatomic.h` is C11 and portable) — `CreateThread` returns only if/when the MSVC target is built; `SetConsoleMode` + `ENABLE_VIRTUAL_TERMINAL_PROCESSING` for the 14 `termios`-family occurrences (the escapes `stdlib.terminal` emits work on Windows 10+); `GetModuleFileNameA` for `readlink`/`_NSGetExecutablePath`; `_ftelli64` (not `ftello` — only `_ftelli64` exists under both toolchains) at `sprout_runtime.c:7241`; stub `getrlimit`. Surfaces with no implementation return the established `"…unsupported on this platform"` shape (precedent `sprout_runtime.c:7676`) — never a silent success.
  ...
  - [ ] `P1` **W5 — link, CI, game.** First `.exe`. Add a `windows-latest` CI job mirroring the existing `macos-latest` job's shape (`.github/workflows/ci.yml:181-210`: bootstrap, then a task/IO smoke against the platform backend). The CI job builds with **clang targeting `x86_64-pc-windows-msvc`**, not mingw — the local loop is mingw, so MSVC must be the gated one or the pure-Win32 rule rots unobserved.
- [ ] `P2` **POSIX `<regex.h>` has no MSVC equivalent** (2026-08-15). One use, `regex_compile_ere` (`runtime/sprout_runtime.c:6039`), reached via `regcomp`/`REG_EXTENDED`. Not in the MSVC CRT. Filed separately from W3 because stubbing it removes a **language-visible** feature rather than an internal capability — the decision is vendor-a-small-ERE-implementation vs. narrow the feature, and it should be made on its own terms.
