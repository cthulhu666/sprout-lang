...
- [~] `P1` **A `Double` does not survive `stringify` → `json_parse`, and above `1e17` does not survive at all (reported 2026-08-20 from `uncharted-suns`'s `game.wire_json`; independently reproduced here on `c90bf97a`). SUB-DEFECTS (a), (c) AND (e) FIXED 2026-08-20; (b) non-finite values and (d) sub-ULP accuracy remain, both policy/accuracy decisions rather than plain defects.** The writer and the reader are different implementations with different capabilities: `json_stringify` is C (`runtime/sprout_runtime.c:6231`, `snprintf("%.17g")`), while `json_parse` reads numbers with the pure-Sprout `parse_double` (`stdlib/json.sprout:112` → `stdlib/prelude.sprout:1452`). The writer emits three things the reader cannot read back. This is **not only a self-round-trip defect** — `1e+17` is a valid JSON number and `%g`-style exponents are what other producers emit, so `json_parse` also rejects standards-conformant JSON from any non-Sprout source. Consumer impact is recorded in `uncharted-suns` `docs/net-stage1-v0.md §5`, where the game gave up on JSON numbers entirely and now ships every `Double` as `JsonInt(double_to_bits(x))`. Sub-defects, each measured, in priority order:
  ...
  - **(b) Non-finite doubles are written as invalid JSON.** `%.17g` renders them `inf` / `-inf` / `nan`, which are bare identifiers no JSON parser accepts — measured: all three stringify, then fail to re-parse. A single divide-by-zero anywhere therefore produces a document that no consumer can read, rather than a visible glitch. JSON has no non-finite number literal, so this needs a policy decision (reject at `stringify` with a loud error, or emit `null`) rather than a parser fix, and the decision is a "Design Change Process" one — it changes what `stringify` may do to a caller's value, so it needs the prior-art survey that process requires (how other languages' JSON writers spell a non-finite number) before the call is made.
