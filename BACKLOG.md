...
- [~] `P2` **A lambda whose parameter type is fixed only by something LATER gets its dictionary resolved too early, and resolved wrong (found 2026-08-20 while fixing the item above). PARTIALLY FIXED 2026-08-20 — the top-level `let` half is closed; see the last paragraph. What remains is a function's declared return type and a local `let … in`, neither of which can carry an annotation to push down.** `fn get_labeller() -> Int -> String = \n -> label(n)` and `let f = \m -> label(m) in f(n)` — for any `where`-constrained `label`, **in a caller with no `where` clause of its own** — are rejected with ``dispatch-verify: dictionary for `ToString a` resolved to `ToString String` but the call fixes a = Int``. **This is not about constrained functions in value position, and it predates that work:** both shapes fail identically, written out by hand, on a compiler without the fix above. The tests for that item deliberately exclude these two positions for this reason.
  ...
  **Still open: a function's declared RETURN type is not pushed down either.** `fn pick() -> Int -> String = label` is rejected with the `dispatch-verify` mismatch above. It is the same fix at a second site, deliberately not bundled: unlike a `let`, the return `TypeExpr` has to resolve through the same parameter-tyvar mapping the body env uses (`fn wrap(x: a) -> a -> String where ToString a = \y -> …`), and this shape is already loud and located, so it is above the floor.
