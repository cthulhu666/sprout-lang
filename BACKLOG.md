...
- [ ] `P0` Fix GC root pool exhaustion (65536-entry limit hit when `--emit-ir` processes non-trivial stdlib modules); root cause not yet confirmed — possible monotonic leak or per-frame spike across deep recursive calls.
