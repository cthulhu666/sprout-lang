...
- [ ] `P3` **Residues of the parenthesised-arrow effect drop.** Surfaced 2026-09-07 while fixing `infer.type_from_ast` (fixtures `tests/conformance/{type_error,run}/effect_paren_arrow_annotation*`). Four remain, all verified by execution. (1) A trailing annotation on a non-arrow type — `Int !{IO}` — has no effect slot and is discarded; spec §7 rule 9 now records that it carries no meaning, but rejecting it outright would be more honest and touches the same constructor as the `!{NOPE}` label check one entry above, so decide them together. (2) Return position cannot spell "returns an effectful function": `parser.parse_return_type_cont` splits `-> (Int -> Int) !{IO}` into return type `Int -> Int` plus FnDecl effect `IO`, i.e. "this function performs IO". Parameter and stored positions express it fine (`f: (Int -> Int) !{IO}`); return position has no spelling for it. (3) Two annotations on one arrow resolve **last-one-wins** and silently: `f: (Int -> Int !{e}) !{IO}` type-checks as `(Int -> Int !{IO}) -> …`, the written `!{e}` gone with no diagnostic, and `(Int -> Int !{IO}) !{}` would downgrade the arrow to pure the same way. Rule 9 admits one annotation per arrow, so this is a rejection case; `types.attach_arrow_effect` has no error channel, which is the reason it is not one yet. **The reachable spelling is an annotated alias**, which is spec §5.6.2's own example shape (`type alias Handler a = Int -> a !{IO}`): given `type alias Handler = Int -> Int !{IO}`, a use site written `h: Handler !{e}` types as `Int -> Int !{e}` (verified 2026-09-07). The alias's declared `!{IO}` is erased by an annotation that reads as merely adding polymorphism, and unlike the nested spelling the overwritten annotation is not visible at the use site at all. (4) An annotation followed by a further `->` does not parse — `(a -> b) !{IO} -> C` fails at the `->` because `parser.parse_type_expr_cont` returns after consuming the annotation without looking for one. Enclosing it works and is correct (`((a -> b) !{IO}) -> C` types as `(a -> b !{IO}) -> C`), so this is a grammar wart, not a missing capability; fixtures `tests/conformance/parse_error/effect_annotation_before_arrow.spr` and the enclosed twin in the run fixture pin both halves.
