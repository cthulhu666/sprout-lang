...
- [ ] `P0` End-to-end dynamic verification: `compile_driver_bin_stage1 --emit-ir` produces valid stage-2 IR for `compile_driver.sprout` without crashing or running out of memory. Stage-1 binary now built (1.52 MB; stage-0 IR emission: 749 MB peak RSS / 51s). Stage-2 self-compile (`compile_driver_bin_stage1 --emit-ir compile_driver.sprout`) still OOMs (exit 137, >4 GB) — blocked on `str_concat` memory leak (runtime `str_concat` mallocs without GC tracking, causing unbounded heap growth during string-heavy IR emission passes). The watchdog correctly prevents host OOM but cannot make memory appear.
...
- [ ] `P0` Add `BootstrapStage3Tests` that runs stage-1 → stage-2 IR → stage-2 binary → stage-3 IR and asserts byte-equality (or structural equality) between stage-2 and stage-3 IR (the fixed-point check). Blocked on `str_concat` OOM fix — stage-2 self-compile exits 137 before any IR is produced.
