...
- [x] `P2` **Enforce the effect rules (spec-v0.md §7 rules 8, 9, 11) — DONE 2026-08-16.** `fn shout(s: String) -> Unit = print(s)` is now a compile error; spec §7's enforcement note is flipped and README's "Effects in types" bullet no longer overclaims. Cost: **zero source annotations, zero correct programs rejected** — the four preceding changes below took the corpus to zero real gaps first, which is what the instrument existed to establish. Enforcement is a POST-PASS in `checker.typecheck_typed` over the reports `infer` already collected, not an error at the declaration boundary, so (a) every gap is named in one compile rather than one-per-compile, and (b) `--phase effects` calls `typecheck_typed_with_effects` directly and keeps enumerating — the census instrument survives enforcement, which matters most when somebody is migrating against it. Both share `unifier.effect_report_is_gap`, now a single definition. Fixtures: `type_error/effect_pure_body_does_io` (fn boundary), `type_error/effect_pure_instance_method_does_io` (instance boundary — separate on purpose; the census's original bug was instrumenting only the `fn` site), `run/effect_over_declared_ok` (subsumption still accepted — a checker that unified rather than subsumed would reject legal code and the type_error fixtures would not notice). `docs/effect-enforcement-v0.md` §11. Rules 9 and 11 followed the same day — this entry originally claimed they were unenforced, which was wrong; see the CORRECTION below. History of how it got there:
  ...
  - **NOT minor and NOT conservative — a live IO-laundering hole. Corrected 2026-09-07.** `infer.call_effect_of` with `argc <= 0` returns `scheme_effects(scheme)` raw — an unfreshened program-level `EffectVar`; and `infer_call_general`'s `arrows_effect(t, argc)` returns `Pure` for `argc <= 0` with no scheme fallback, so a zero-arg call on an *expression* callee loses its effect. This entry previously called both "conservative (accept, do not reject)". Accepting a program that runs IO under a pure signature is not conservatism, and the shape needs no record field or `if` — a plain `let` reaches it: `fn launder() -> Int = let t = io_thunk in t()` where `io_thunk() -> Int !{IO}` **compiles, links and prints**, with `--phase effects` reporting `declared pure, inferred pure`. The direct spelling `= io_thunk()` is correctly rejected by rule 8, so the hole is precisely the local/expression callee. Distinct from every other known effect hole in kind: the effect is **dropped before any comparison happens**, so no check that compares two effects can see it — confirmed by a compiler instrumented to reject every concrete pure/IO arrow meet, which reports zero errors on it. Fix is to read the arrow's effect at `argc <= 0` instead of returning `Pure`. Tracked as the fourth boundary in `docs/effect-subsumption-v0.md`; "the census shows nothing in-tree hits either" remains true and remains irrelevant to soundness.
...
- [x] `P1` **RESOLVED (not reproducible) 2026-08-21.** `wrap` value rendered as garbage / crash via user `ToString` + `${x}` interpolation. Surfaced 2026-07-02 while adding daemon logging. Repro (`/tmp/repl_repro.txt`): `wrap Minutes = Int` + `instance ToString Minutes { fn to_string(x) = ` + "`${x} minutes`" + ` }` + `let m = Minutes(10)`, then eval `m`. Expected `10 minutes`; the eval path prints `\xNN\xNN minutes` — the wrapped `Int` value `10` is rendered as if it were a heap `String` pointer (deref of `10` as `char*` → garbage bytes). Same fault crashes the interactive REPL daemon (`analysis service: empty response`) when the corrupt pointer is unmapped; non-interactively it merely garbles output. Reproduces deterministically via `SPROUT_ANALYSIS_SERVICE_LOG=/tmp/x ./build/sproutd < /tmp/repl_repro.txt`. Likely a representation/dispatch fault in the compiler core (`ast_to_ir`/`codegen` `to_string`/interpolation lowering for `wrap`-typed args) rather than a rooting bug — `${x}` where `x : Minutes` should dispatch through the user `ToString Minutes` instance but appears to route the underlying-`Int` value into the string-rendering path. Shared-core bug: affects any consumer of the compiler (REPL, analysis daemon, LSP). **Re-triaged 2026-08-21 on `4ec7b087`: does not reproduce on either path, and the repro as filed was not a well-founded program.** A valid version — `wrap Minutes = Int` with `instance ToString Minutes` unwrapping via a `match` on the constructor and rendering the unwrapped `n` — produces `10 minutes` identically from `to_string(m)`, from `${m}`, and from `++`, both compiled and through `sproutd`'s eval path (bare `m`, `to_string(m)` and an embedded `${m}` all correct, no daemon crash). The repro as written cannot run at all: its instance body interpolates `x` itself (`x : Minutes`), and `${x}` desugars to `to_string(x)` — the very method being defined — so it is unbounded recursion, and the expectation of `10 minutes` assumes `${x}` renders the *underlying* `Int`, which is not what a `wrap` carrying its own instance means. Both paths now reject it cleanly instead of emitting the corrupt pointer. **Residual, already filed elsewhere:** that rejection is B2's message (`No instance of <Class> for <T> in instance method <m>`, the `P2` below on recursive instance-method dispatch) — the instance plainly exists, and the same message hits legitimately self-recursive instance methods; the diagnosis belongs there, not here. Guarded going forward by `tests/stdlib/test_wrap_tostring_interpolation.spr` — two wraps over the same `Int` with different instances, so "dispatched on the wrap" is distinguished from "dispatched on `Int` and formatted by luck", plus the forwarded-dictionary path. That test closed a real gap: `examples/wrap_typed_money.sprout` covers `wrap` + `ToString` but renders with `++`, so the interpolation path had nothing on it.
...
- [x] `P2` **Double transcendental math + Int/Double module split. LANDED 2026-08-06.** Design of record: `docs/math-transcendental-v0.md`; measurements: `bench/results-2026-08-06-math-transcendental.md`. `stdlib.math` is now the `Double` layer (`fabs`→`abs`, `fclamp`→`clamp`, plus new `exp`, `ln`, `log2`, `log10`, `log(x, base)`, `cbrt`, `pow(Double, Double)`); the `Int` surface moved verbatim to `stdlib.math.int`. All pure Sprout — **no new builtin, `APPROVED_BUILTINS` untouched** — at ~1e-13 relative accuracy, with `pow` exact for integer exponents *whenever every intermediate product is exact*, which covers Stefan-Boltzmann `T⁴`. (This entry originally said "bit-exact for integer exponents" without qualification. That is the false claim retracted in the post-merge-review entry below and in `docs/math-transcendental-v0.md` §11.5 — corrected here too, since an unqualified version surviving in the backlog is how it would get requoted.) Unblocks Tsiolkovsky Δv (needed `ln`) and sigmoid/softmax (needed `exp`, which `docs/nn-gap-analysis.md` had wrongly claimed required a C extern — corrected there). Zero compiler change: `module_name_to_path` maps dots to slashes, and `stdlib/compiler.sprout` beside `stdlib/compiler/` is precedent. Follow-ups below.
  - [x] `P2` **`Eq Double` / `Ord Double` — LANDED 2026-08-29: `docs/eq-ord-double-v0.md`.** The doc supersedes `numeric-types-v1-draft.md` §7.1, whose framing rests on a premise that is false for Sprout — the `==`/`<` OPERATORS and the `Eq`/`Ord` CLASSES are already separate code paths and neither can reach the other (`infer.check_eq` short-circuits on `is_primitive_eq_type`, which lists `Double`; `check_compare` has no class fallback for any type). Verified in emitted IR: with both instances in scope, `x == y` on `Double` is `fcmp oeq` while `eq(x, y)` is `call @__tc_Eq_Double_eq`. So a total `Ord Double` does NOT make the operators lie, and the prior-art consensus (5 of 6 surveyed languages) is exactly this split — only Haskell conflates them, and GHC's own source concedes its `Eq Double` "does not satisfy reflexivity". Recommendation is option A (total instances in the class, operators untouched, NaN greatest per Java/Julia); a prelude spike passed 19/19 including `vec_sort` over a list with NaN, and `just ir-golden-diff` moved **1 of 60** files (DCE drops unused prelude instances everywhere else). RED test on disk: `tests/stdlib/test_double_ord.spr`. Original entry follows. **the decision `numeric-types-v1-draft.md` §7.1 said was "needed before `Double` ships", and `Double` shipped without it.** `Double` has exactly one instance in the whole stdlib (`ToString`, `prelude.sprout:814`). Live consequences today: `check_eq` does not type-check on `Double`, so `stdlib/test.sprout` carries `check_approx`/`check_approx2` as a workaround; and `stdlib.math` deliberately ships **no** `Double` `min`/`max`/`sign` because they would force the question. The blocker is real, not neglect — IEEE NaN is unordered, so any total `Ord Double` is a lie. Prior art diverges: **Rust** gives `f64` only `PartialOrd`, never `Ord`; **Haskell** defines `Ord Double` and it is a well-known footgun (`compare NaN x` misbehaves, breaking `maximum`/`sort`). The draft's §7.1 lists three options (inherit IEEE / add `PartialOrd` as a superclass split / wrap in `Maybe`). Decide before adding any `Double` comparison-based API.

    > **New consequence, 2026-08-27 — this now blocks a shipped API, not just a hypothetical one.** `minimum`/`maximum`/`min_by`/`max_by` landed on `where Ord k` (`docs/min-max-combinators-v0.md`), so a `Double`-keyed argmin does not type-check and must still be hand-folded. That is not a corner case: `uncharted-suns` open-codes this fold at `game/render_vista.sprout:1285` (nearest-pick by squared distance), `game/stations.sprout:70-80` (innermost in-band semi-major axis), `game/economy.sprout:715` (`argmax5` over supply sums) and `loam/hydrology.sprout:437` — **all four keyed on `Double`** — plus a hand-written `fn min_by(score: a -> Double, xs: List a) -> Maybe a` in `grimward/combat.sprout:200` whose own comment names the prelude gap as its reason to exist. Resolving `Ord Double` retires all five with no change to the combinators' signatures. **CORRECTED 2026-08-29: it retired ONE, not five.** The count above was taken from grep hits, not from reading the code, and only `grimward/combat`'s is a `min_by`. `argmax5` keeps the FIRST of equal keys where the prelude's `max_by` keeps the LAST, so it is not behaviour-neutral; `stations.habitable_go` is a *filtered* argmin returning an index with a sentinel; `hydrology`'s and `render_vista`'s are effectful, and `Foldable.fold_values` has no effect row, so they were never candidates. The `deriving`-virality consequence below was always the larger half and is unaffected. The sequencing was a deliberate call (design doc §9, option 1): ship the `Int`/`String`/tuple/ADT coverage now rather than block four functions behind the NaN decision.
    >
    > **Two further consequences, measured 2026-08-29 — the gap is wider than the combinator count suggests and it GROWS on its own.** (a) `deriving (Eq, Ord)` is impossible for **any record or ADT containing a `Double` field**, virally: derived bodies call the class methods per field, so `type Reading = (label: String, value: Double) deriving (Eq, Ord)` fails with `No instance of Eq for Double in instance method eq`. That is not a corner of the stdlib, it is every user type with a coordinate, a mass, or a score in it. (b) The `member` combinator that landed 2026-08-29 carries `where Eq a` and so inherited the hole on arrival — `member(1.0, [1.0, 2.0])` does not typecheck, and `docs/collection-combinators-v0.md` never mentions `Double` at all. Nobody decided to leave `member` broken on `Double`; every future `Eq`/`Ord`-constrained combinator falls in by default. Note the shape this gives the prelude: on current master `Double` is the only primitive that can be filtered and searched but not compared, deduplicated, or extremised. Also worth pricing into the decision: `numeric-types-v1-draft.md:384` records a *preference* for a `PartialOrd` split, and that option does **not** deliver min/max on `Double` — it formalises the exclusion, and delivering the request under it would mean re-signing the four landed combinators to `where PartialOrd k` and answering the NaN question anyway.
  - [ ] `P2` **The comparison operators are the only CLOSED operator family, and they silently default an unresolved operand to `Int`.** Surfaced 2026-08-29 while scoping `Ord Double`. `check_compare` (`infer.sprout`) *unifies* the operand type against `Int`, then `Char`, then `Double`, and hard-errors otherwise. `check_eq` instead *tests* the resolved type (`is_primitive_eq_type(apply_subst(…))`) and falls back to the `Eq` class; `++` falls back to `Semigroup`. So `<` `<=` `>` `>=` alone can reach no class, and `Ord`'s ten instances (`String`, tuples, `Maybe`, `List`, `Result`, `Vec`, …) are unreachable from the operators — `"a" < "b"` is unimplementable and reports `< needs Int, Char, or Double: Type mismatch: String vs Double`, where the `Double` half is just whichever attempt happened to be last. Two verified consequences, both worse than the missing capability: `fn lt(x, y) = x < y` silently infers `Int -> Int -> Bool` with the error surfacing at the *caller*; and `fn lt2(x: a, y: a) -> Bool where Ord a = x < y` is REJECTED with `Signature too general for its body in lt2: type variable a forced to Int` — the compiler overrides an explicit `Ord a` promise and then blames the signature. Unification against a type variable succeeds and binds it, which is the mechanism. **Note the fix is not free**: making `check_compare` non-destructive would reject `fn lt(x, y) = x < y`, which compiles today, so this is a breaking change to numeric defaulting needing a migration story and a measured breakage count — not the contained bug fix it first looks like. Interacts with the `Ord Double` item above: if `<` ever gains an `Ord` fallback *and* `Ord Double` lands, `x < y` on `Double` would start routing through the class and change meaning at NaN, which is exactly Haskell's footgun. **`Ord Double` has since landed (2026-08-29), so this is now a hard constraint rather than a coupling: any `Ord` fallback added to `check_compare` MUST keep the existing `Double` short-circuit.** The whole basis of `docs/eq-ord-double-v0.md` is that the operator and the class are separate paths — a fallback that let `<` reach `Ord Double` would silently make `nan < 1.0` true and retroactively turn that design into Haskell's. The `Int`-defaulting half of this item is unaffected and still open.
  - [ ] `P3` **Export `is_finite` / `is_infinite` from `stdlib.math`.** The module exports `is_nan` (`math.sprout:282`) and keeps a private `inf = 1.0 / 0.0` (`:504`), but there is no way for a caller to ask whether a `Double` is writable/representable without rebuilding the predicate. `stdlib/json.sprout` needed exactly that for the non-finite refusal (§JSON Support (b)) and defined its own three-comparison `is_finite` rather than importing `stdlib.math`. The reason is per-program bundling, NOT the bootstrap seed: `stdlib.json` is outside `compile_driver`'s import closure, so the dependency never reaches `bootstrap/compile_driver.ll`. Measured on a trivial json-only program, adding `stdlib.math` costs 20178 → 24476 IR lines and 419 → 486 functions (+21%). That is the right call for that module and the wrong shape for the stdlib overall: the predicate now exists twice. Exporting it from `math` does not by itself let `json` use it, so this is worth doing for third-party callers, and worth revisiting for `json` only if the prelude ever grows a finiteness predicate (which is a separate always-bundled-surface decision).
  - [ ] `P3` **`Double` `min`/`max`/`sign`.** **Unblocked 2026-08-29** — `Ord Double` landed, so these are now writable (generically, via `compare`, or monomorphically). Still note that C's `fmin`/`fmax` are *not* simply `if a < b` — they return the non-NaN operand when exactly one is NaN, which is a deliberate deviation from IEEE ordering that a naive implementation gets wrong, and which also differs from the prelude's `Ord Double` (NaN greatest, so `compare`-based `max` would *propagate* the NaN where C's `fmax` discards it). Pick one and say which in the doc comment; do not let the two definitions be chosen by accident. Prior art is split: C99/Rust `f64::max` discard NaN, OCaml's `Float.max` propagates it.
...
- [~] `P1` **A `Double` does not survive `stringify` → `json_parse`, and above `1e17` does not survive at all (reported 2026-08-20 from `uncharted-suns`'s `game.wire_json`; independently reproduced here on `c90bf97a`). SUB-DEFECTS (a), (b), (c) AND (e) FIXED 2026-08-20. (d) sub-ULP accuracy remains, an accuracy limit rather than a plain defect; (f) out-of-range literals reading as an infinity was found while verifying (b) and is a range-policy decision.** The writer and the reader are different implementations with different capabilities: `json_stringify` is C (`runtime/sprout_runtime.c:6231`, `snprintf("%.17g")`), while `json_parse` reads numbers with the pure-Sprout `parse_double` (`stdlib/json.sprout:112` → `stdlib/prelude.sprout:1452`). The writer emits three things the reader cannot read back. This is **not only a self-round-trip defect** — `1e+17` is a valid JSON number and `%g`-style exponents are what other producers emit, so `json_parse` also rejects standards-conformant JSON from any non-Sprout source. Consumer impact is recorded in `uncharted-suns` `docs/net-stage1-v0.md §5`, where the game gave up on JSON numbers entirely and now ships every `Double` as `JsonInt(double_to_bits(x))`. Sub-defects, each measured, in priority order:
  - **(a) FIXED 2026-08-20. A `+`-signed exponent is rejected outright — the hard cliff.** Fix: a new private `parse_exponent` (`prelude.sprout`) accepts the sign the exponent is allowed to carry, by accumulating from index 1 past a leading `+`. **`parse_int` was deliberately NOT changed** — a `+` is legal only in the exponent, so teaching `parse_int` about it would have made `parse_double("+1.5")` and the JSON reader accept a form RFC 8259's grammar forbids. Reusing the existing digits-only `accum_digits` for the post-`+` remainder also keeps `1e+-3` and a bare `1e+` rejected, which a naive strip-the-plus would have silently accepted as `1e-3`. Both cliffs are gone: text and JSON now round-trip at 1e15/1e17/1e18/1e21. Tests: 7 cases in `test_parse_double.spr` + 5 in `test_json.spr` (RED-verified against the unfixed prelude), plus 4 guard cases pinning that a `+` on the mantissa, a dangling `1e+`, and a double-signed `1e+-3` all stay rejected. Original report follows. `split_exponent` (`prelude.sprout:1437`) hands the exponent text to `parse_int`, whose `accum_digits` (`:1382`) accepts a leading `-` but treats `+` as a non-digit and returns `Nothing`; the whole parse then fails. So every number `%g` renders in exponential form with a positive exponent is unreadable, while negative exponents (`1e-05`) are fine — the cliff is one-sided. It lands at a **different magnitude per path**, because the two writers use different precision: `double_to_string` starts at `%.15g` (`sprout_runtime.c:2811`) so text round-trips die at **1e15**; `json_stringify` uses `%.17g` so JSON round-trips die at **1e17** — the reported number. Measured: `1e14` text `100000000000000.0` → ok; `1e15` text `1e+15` → `Nothing`; `1e16` json `10000000000000000` → ok; `1e17` json `1e+17` → parse error. Fixing `accum_digits` to accept a leading `+` in the exponent (JSON and IEEE 754 both permit it) closes the cliff on both paths and is a prelude-only change.
  - **(b) FIXED 2026-08-20 (policy decision by Kuba after a prior-art survey): non-finite doubles were written as invalid JSON; `stringify` now refuses them.** `%.17g` rendered them `inf` / `-inf` / `nan`, bare identifiers no JSON parser accepts — measured on `0b828f8d`: `JsonFloat(nan)` → `nan`, `+inf` → `inf`, `-inf` → `-inf`, an object containing one → `{"k":inf}`, and all three then fail to re-parse. A single divide-by-zero anywhere therefore produced a document no consumer could read, rather than a visible glitch. **Decision: `stringify(value: Json) -> Result JsonError String`, returning `Err(JsonNonFinite x)`.** RFC 8259 §6 states outright that values outside the `number` grammar, "such as Infinity and NaN", are not permitted, so a writer must choose; the survey (each row verified against a primary source or run locally, 2026-08-20) found genuine divergence but a sharp negative consensus:

    | library | NaN | ±Infinity | mechanism |
    |---|---|---|---|
    | JavaScript `JSON.stringify` | `null` | `null` | silent (verified empirically on V8) |
    | Rust `serde_json` | `null` | `null` | silent (`ser.rs`, `FpCategory::Nan \| Infinite → write_null`) |
    | Haskell `aeson` | `null` | `"+inf"` / `"-inf"` strings | silent, asymmetric (`realFloatToJSON` / `realFloatToEncoding`, agreeing on both paths) |
    | Java Jackson | `"NaN"` | `"Infinity"` quoted | `WRITE_NAN_AS_STRINGS`, "enabled by default" |
    | Go `encoding/json` | error | error | "NaN and +/-Inf values will return an UnsupportedValueError" |
    | .NET `System.Text.Json` | throw | throw | `JsonWriterHelper.ValidateDouble` → `ArgumentException`; named literals opt-in via `AllowNamedFloatingPointLiterals` |
    | Swift `JSONEncoder` | throw | throw | `NonConformingFloatEncodingStrategy.throw`, "This is the default strategy" |
    | Ruby `JSON.generate` | raise | raise | `JSON::GeneratorError: NaN not allowed in JSON` (verified locally) |

    Roughly 4 refuse / 3 coerce to `null` / 1 quotes. **Of the eight, exactly one entry point emits a bare non-finite literal by default — Ruby's `JSON.dump`, which passes `allow_nan: true`, while Ruby's own `JSON.generate` raises.** That bare literal was Sprout's only behaviour. The tiebreaker was internal rather than a headcount: `null` and a quoted `"NaN"` both come back a *different* `Json` constructor than went in, which is exactly the silent round-trip type change (e) above was fixed to remove, one commit earlier. A `tcp_fail` abort was rejected too — library code calling `exit(1)` is the C1 bug class already filed and fixed in `http_server`; Go returns an error, it does not panic. `Result` is the only option that is both loud and non-fatal, and `parse` already had that shape. Implementation: a module-private `is_finite` plus a `first_non_finite` walk in `stdlib/json.sprout`, so the C writer is unreachable with a non-finite Double and its branch now documents that as an invariant — **no new builtin, and `stdlib.math` deliberately not imported** — not for the reason first assumed. `stdlib.json` is *outside* `compile_driver`'s import closure (the reseed for this change moved exactly one line of `bootstrap/compile_driver.ll`, the fingerprint), so the dependency would never reach the seed. The cost is per-program bundling instead, and it was measured: on a trivial json-only program, adding `stdlib.math` goes 20178 → 24476 IR lines and 419 → 486 functions (+21%), 0.65s → 0.82s to compile, for a three-line predicate. New `JsonError` variant `JsonNonFinite Double` carries the offending value; new `json_error_message(err) -> String` lets callers report one without enumerating variants. Tests: 18 cases in `test_json.spr`; RED-verified twice — behaviourally against the unfixed tree (5 assertion failures, no compile error), then by mutating the guard to always-true (7 refusal tests fail) and always-false (15 acceptance tests fail, including the `-0.0` whose sign (c)/(e) exist to preserve).
  - **(c) FIXED 2026-08-20 in `parse_double`; still not observable through JSON because of (e) below. `-0.0` loses its sign on the way in.** Fix: apply the sign with unary `-`, which lowers to `fneg` (`ast_to_ir.sprout:1064` dispatches unary minus on the operand type precisely so a `Double` gets an IEEE sign flip rather than a bit-pattern `sub`). `parse_double("-0.0")`, `"-0"` and `"-0.0e5"` now all carry the sign bit; verified by comparing `double_to_bits` against a `(0.0 - 1.0) * 0.0` reference, since `approx` cannot distinguish two numerically-equal zeros. **`json_parse("-0.0")` is fixed too — but `stringify(JsonFloat(-0.0))` writes `-0`, which re-reads as a `JsonInt`, so the JSON round trip still shows `0`; that is (e), a writer defect, not this one.** Original report follows. The writer emits `-0` correctly; `parse_double` returns `+0.0`, because its negation is arithmetic — `if neg then 0.0 - (mag * pow10(e))` (`prelude.sprout:1460`) — and for `mag = 0` that is `0.0 - 0.0`, which IEEE 754 defines as `+0.0` under round-to-nearest. Measured: text `-0.0` → `0.0`, json `-0` → `0`. Negating the *result* (an IEEE sign flip) rather than subtracting from zero fixes it; note `spec-v0.md:1615` already states bit access is the only way to distinguish the two zeros, so this is a genuine value change, not a formatting nicety.
  - [ ] `P2` **(f) An out-of-range literal reads as an infinity, so a conformant document can be READ but not re-written.** Surfaced 2026-08-20 while tracing (b)'s reachability into `uncharted-suns`. `parse_double` builds its value as `mag * pow10(e)`, and `pow10` overflows to `+inf` past the Double range, so an over-large magnitude returns `Just(inf)` rather than `Nothing`. Measured through the public API: `parse("1e400")` -> `JsonFloat(+inf)` -> `stringify` -> `Err(JsonNonFinite)`; `parse("-1e400")` likewise. `1e-400` underflows silently to `0.0` (standard IEEE, and what every surveyed implementation does). RFC 8259 §6 names `1E400` specifically as an interoperability hazard and explicitly *allows* an implementation to "set limits on the range and precision of numbers accepted", so rejecting it at parse is a legitimate option that has not been taken. **This is a consequence of (b), not a regression from it** — before (b) the same document round-tripped to a bare `inf`, which no reader accepts, so it was broken further downstream and more quietly. It is also the only mechanism found by which a non-finite Double can enter a Sprout program from ordinary JSON input rather than from arithmetic. The decision is a Design Change Process one (it changes what `parse` accepts), and the options are: reject an out-of-range magnitude as `Err`, keep the saturating `inf` and document it, or clamp to the largest finite Double. Note whichever is chosen should also settle the silent underflow-to-zero, which is the same question at the other end. No caller is known to depend on the current behaviour.

  - **(d) Sub-1.0 magnitudes lose the low bit.** `parse_mantissa` computes `iv + (fv / pow10(len(frac)))` (`prelude.sprout:1432`), so a value is rounded twice — once forming `fv`, once dividing — and lands 1 ULP off. Measured over 200 pseudo-random mantissas per decade, comparing `double_to_bits` after a `stringify`/`json_parse` round trip: **1e-9 → 85/200 fail, 1e-3 → 26/200 fail**, while 1e0 … 1e16 are 0/200 and 1e17 is 200/200 (that last is (a), not rounding). `uncharted-suns` measured the same shape independently (22–26% per decade over 1000 samples). This is the documented limit of a pure-Sprout reader (`prelude.sprout:1449`: "NOT a bit-exact strtod round-trip") and closing it means either a correctly-rounded decimal→binary algorithm in Sprout or a `strtod`-backed builtin — the latter needs approval under "Builtin vs Stdlib" rules 4–6, with the correctness argument (not performance) doing the work.

  - **(e) FIXED 2026-08-20 (approved by Kuba as a deliberate wire-format change). Found while verifying the (c) fix: an integral-valued `JsonFloat` round-trips into a `JsonInt` — the value's TYPE changes.** `json_stringify`'s `JsonFloat` branch (`sprout_runtime.c:6231`) writes raw `%.17g`, so `JsonFloat(2.0)` is written `2`; the reader classifies a token with no `.`/`e`/`E` as an integer (`json.sprout:111`), so it comes back `JsonInt(2)`. Measured by inspecting the returned constructor, not the rendering: `2.0` → `"2"` → `JsonInt`; `2.5` → `JsonFloat`; `-0.0` → `"-0"` → `JsonInt`; `1e17` and `0.1` → `JsonFloat`. This is why (c) is invisible through JSON — `-0` is just the integral case that also loses a bit of information. **The reader cannot fix this**: JSON has no int/float distinction, so `-0` genuinely carries no evidence it was a `Double`, and the `.`/`e` heuristic is the only signal available. The fix belongs in the writer, and the same file already contains it — `double_to_string` appends `.0` when `%g` yields a bare integer, with the comment "so a Double never reads as an Int" (`sprout_runtime.c:2819-2830`); the JSON writer simply does not apply that rule. **This changes the wire format** — a `Double`-typed field that serialized as `{"x": 2}` now serializes as `{"x": 2.0}` — so it was put to Kuba rather than taken as a drive-by, and approved on the grounds that a silent type change across a round trip is worse than a wider rendering. Fix: the `.0`-appending rule was extracted from `double_to_string` into a shared `append_dot_zero_if_bare_int` (`sprout_runtime.c`) and applied in both writers, so the two can no longer drift apart — the whole defect was one of them having the rule and the other not. Non-finite values are excluded by the same letter check that already guarded `double_to_string`, so (b) is untouched and still open. Tests: 8 cases in `test_json.spr`, RED-verified against the unfixed runtime (3 fail without it). Note the strict assertion is `json_get_int` returning `Nothing`, **not** `json_get_float` returning `Just` — `json_get_float` deliberately coerces a `JsonInt` (`json.sprout:365`), so it answers `Just` either way and cannot see this collapse. Original report follows.

  Regression coverage for the two landed fixes lives in `tests/stdlib/test_parse_double.spr` and `tests/stdlib/test_json.spr`; both sets were confirmed RED against the unfixed prelude before the fix went in. The remaining three — (b), (d), (e) — each need their own first-failing test with their fix, and **each is a policy or accuracy decision rather than a straightforward defect**: (b) and (e) change what the writer emits, and (d) needs either a correctly-rounded decimal→binary algorithm in Sprout or a `strtod`-backed builtin (approval required under "Builtin vs Stdlib" rules 4–6, on correctness grounds — performance would not justify it).
...
- [x] `P0` Add key input primitive. **CLOSED 2026-09-05 by the TUI M0 change** (`stdlib/tui/` + `ide/` arc). The scoping note this entry ends with — "making stdin a parking-capable descriptor is a runtime change and needs sign-off under Builtin vs Stdlib rules 5–6" — was taken up and signed off; four builtins landed in `stdlib/terminal.sprout`: `term_raw_enter`/`term_raw_exit` (session raw mode + SIGWINCH + an `atexit` restore), `term_size` (`TIOCGWINSZ`), and `term_read_avail(max, ms) -> TermInput` (parks the calling task via `scheduler_park_on_fd_timeout`). Decoding moved OUT of C: a caller now gets raw `Bytes` and decodes in Sprout, which also makes the decoder testable for the first time — the audit below notes the input half was the part `tests/conformance/run/terminal_escapes` could not cover. `term_read_key` is unchanged and still used by `stdlib/repl.sprout`; the new surface is additive. Two findings worth keeping: (a) the park must use `scheduler_park_on_fd_timeout`, the **plain** entry point, NOT `scheduler_park_on_unowned_fd_timeout` — "unowned" means *no handle table owns the fd, so the parked frame is its only reference and the drop must close it*, so the unowned variant CLOSES the fd when `with_timeout`/`scope_cancel` force-drops the parked task (`sprout_scheduler.c:1046` sets `park_close_fd`, `:709` closes it). Closing stdin is unrecoverable, and worse than a plain leak: fd 0 becomes the lowest free descriptor, so the next `open`/`socket`/`accept` is handed it and a later read silently reads a stranger. The name reads like a safety property and is the opposite of one; `sprout_scheduler.h:115-118` is the authority. Caught by review, not by tests — every stdin fixture *completes* its read, and a completed read is never force-dropped, so `park_close_fd` was not consulted in any of the 43 cases. `tests/task_io_smoke/stdin_cancel_keeps_fd.spr` is the fixture that enters that transition, asserting the descriptor survives rather than that the read worked; (b) pollability has to be an **allowlist** (fifo/socket/tty), not a denylist — `/dev/null` is a character device like a tty but kqueue's `EVFILT_READ` refuses to register it, and `sprout_poll_add` aborts rather than returning an error, so "not a regular file" crashes and "not a character device" excludes the tty. Covered by `tests/task_io_smoke/stdin_park.spr` (order-based RED/GREEN, so a regression reports as a wrong order rather than a hang; also gets Linux/epoll coverage through `just linux-smoke`) and the extended `tests/stdlib/test_terminal.spr`. **Original audit follows, retained because its diagnosis is what scoped the fix. Audited 2026-08-15.** `term_read_key` drives termios raw mode, decodes arrow keys / ctrl-chords / multi-byte UTF-8, and bounds continuation reads with `VMIN=0`/`VTIME=1` so a truncated escape sequence yields U+FFFD instead of hanging. What is missing is not merely a poll flag: `term_read_key` is a plain blocking `read(2)` that **does not park in the scheduler**, unlike `tcp_accept` (cf. the parking note at `runtime/sprout_runtime.c:158`), so in a multi-task program a key read holds the scheduler thread rather than yielding like every other blocking operation in the language. The only `poll` loop in the runtime is the process-spawn one; there is no stdin readiness path. Today's workaround is a blocking event loop (what `examples/sentry_issue_browser_tui.sprout` does), which is fine for purely input-driven TUIs and fails as soon as a UI wants timers, animation, or concurrent network alongside input. **Scoping note: making stdin a parking-capable descriptor is a runtime change and needs sign-off under "Builtin vs Stdlib" rules 5–6 before implementation.** The input side is also the half of `stdlib.terminal` that the new golden-stdout test cannot cover (it depends on stdin and `isatty`).
- [x] `P1` Add line wrapping / viewport helpers in stdlib. **Wrapping LANDED 2026-09-06 as TUI
  M3's pure half**, in `stdlib/tui/text.sprout`: `wrap_to` (greedy, breaks at spaces,
  hard-breaks a word with no break opportunity, treats an existing newline as a hard break),
  plus `width` and `truncate`. All three measure in **columns** via `stdlib.unicode.width` over
  grapheme clusters, which is the reason they are TUI modules and not `stdlib.string` —
  a CJK ideograph is two columns and a combining mark is zero. The cluster segmentation that
  was private inside `screen.sprout` moved here and is now shared with the placer, so one
  definition answers "how wide is this"; two copies could drift, and the failure mode is a
  cursor that walks rather than an error. Named `wrap_to` because `wrap` is the newtype
  keyword. 24 assertions in `tests/stdlib/test_tui_text.spr`.
  - Landed alongside it: `stdlib/tui/layout.sprout` — `Dimension = Cells | Fraction | Auto`
    with an explicit shrink priority (Fraction yields, then Auto, then Cells) and
    largest-remainder rounding so an allocation sums to **exactly** the extent given; `row`,
    `column`, `grid`, `dock`/`dock_all`; and `split_right`/`split_bottom` added to `geometry`.
    Design and the verified prior-art survey (CSS Grid, Textual, ratatui):
    `docs/tui-widgets-v0.md`. 37 assertions in `tests/stdlib/test_tui_layout.spr`.
  - **Original M2 note retained**, since it scoped this:  **Viewport helpers LANDED 2026-09-06 as TUI M2**; wrapping deliberately did NOT land. `stdlib/tui/geometry.sprout` is the viewport half — `Region` with half-open edges, `intersect`, `contains`, `clamp_offset`, `split_left`/`split_top` — and `stdlib/tui/screen.sprout` is a double-buffered grid whose `diff_to_ansi` emits only changed runs. `screen_write` **stops at the right edge rather than wrapping**, on purpose: wrapping is a layout decision that depends on where the text is allowed to flow, and a screen that silently reflowed would hide the caller's bug instead of reporting it. Wrapping belongs to M3's layout engine, where a `Region` is available to wrap *within*.
  - Also landed here: `stdlib/tui/style.sprout` (`Color`/`Style` and an absolute SGR encoding), `stdlib/tui/event.sprout` (the event vocabulary), and `stdlib/tui/keys.sprout` (below). Design: `docs/tui-core-v0.md`.

- [x] `P1` **Escape-sequence decoding moved out of C — LANDED 2026-09-06.** `keys.decode : Bytes -> (List Event, Bytes)` replaces `term_read_key`'s C decoder in practice. That decoder recognises exactly `ESC [ A/B/C/D`; its own comment (`sprout_runtime.c:3811`) records that it "leaks the tail bytes of anything longer back to the caller as separate fake keypresses", so ctrl-right (`ESC [ 1;5C`) arrived as Escape plus three junk keys. The Sprout decoder parses CSI framing, so an unrecognised sequence is **consumed whole** rather than leaked. 80 unit cases plus a `tests/conformance/run/tui_frame` golden — the input half of `stdlib.terminal` was previously untestable, which `terminal_escapes.spr:10-13` says outright; a pure function over bytes is not.
  - `term_read_key` is unchanged and still used by `stdlib/repl.sprout`. Removing it is a separate change with its own consumers to migrate.

- [ ] `P2` **The kitty keyboard protocol.** Would remove the lone-ESC ambiguity outright — the kitty spec states the problem exactly: "No reliable way to distinguish single `Esc` key presses from the start of a escape sequence. Currently, client programs use fragile timing related hacks for this, leading to bugs." It would also deliver meta/super/hyper and key-release events, which the legacy encoding cannot express and which is why `event.Mods` has exactly three flags. Cost is a capability negotiation, and the legacy decoder is still required as the fallback, so this is additive rather than a replacement.
...
- [x] `P2` **`Foldable`-generic extreme selection — `minimum` / `maximum` / `min_by` / `max_by`. LANDED 2026-08-27.** Design and prior-art survey in `docs/min-max-combinators-v0.md`; normative text in `docs/spec-v0.md` §8.5. `_by` takes a **key function** (matching `vec_sort_by` / `vec_sum_by` / Elm's `sortBy`, diverging from Haskell's comparator-taking `minimumBy`); all four return `Maybe a`, so empty is total rather than a panic; ties keep the **first** minimum and the **last** maximum, which makes the pair exactly the ends of `vec_sort_by`'s stable order — pinned as a law in `tests/stdlib/test_min_max.spr`, not just as examples. Coverage caveat and its consequences are recorded on the `Ord Double` item in §1.
- [ ] `P3` Deferred non-goals from `docs/min-max-combinators-v0.md` §2, both waiting on a concrete caller rather than on any blocker. (a) **Comparator-taking `min_with` / `max_with`**, mirroring Elm's `sortWith` — the stdlib has no `sort_with` either, so if comparator forms are ever wanted they should arrive as a set, not one-off. (b) **A generic two-argument `min(x, y)` / `max(x, y)` over `Ord`** — `stdlib.math.int` already has `Int`-specific `min`/`max`/`clamp`, so generalising them is a naming-collision question (a prelude-global `min` would be shadowed by `import stdlib.math.int (min)`, and shadowing a prelude name misdirects the resulting error) before it is an implementation one.
- [x] `P2` **Container-preserving generic `filter` — `class Filterable`. LANDED 2026-08-29.** Design and prior-art survey in `docs/filterable-v0.md`; normative text in `docs/spec-v0.md` §8.5. `filter(pred, xs) -> c a where Filterable c` over `List` and `Vec` — the same instance pair as `Foldable`. It is a **separate class, not a `Foldable` derivation**, because the class variable has to appear in the *result*: `fold_values` only tears a container down, so a fold-derived filter would return one fixed type and turn `Vec`-in into `List`-out mid-`|>`-chain. No superclass (PureScript requires `Compactable`+`Functor`, witherable `Functor`; neither derives a method here, so it would only widen the dictionary). The old List body is now `export fn list_filter`, giving `filter` the `list_` twin every other List helper already had. Instances delegate to `list_filter`/`vec_filter` rather than recursing, per B2.
- [x] `P2` **Collection combinators — the Foldable/Filterable split. LANDED 2026-08-29.** Design and prior-art survey in `docs/collection-combinators-v0.md`; normative text in `docs/spec-v0.md` §8.5. Six `Foldable` consumers as plain free functions (`any`, `all`, `find`, `find_map`, `count`, `member`) plus two new `Filterable` methods (`filter_map`, `partition`) with `list_filter_map`/`list_partition`/`vec_partition` behind them. The organising rule, which every surveyed language shares: **split by what the verb returns, not by which container it came from** — consumers return a scalar or `Maybe` and need only the source (`Foldable`), producers return the same container and need the class keyed on it (`Filterable`). Both new methods are methods rather than derivations *because Sprout is strict*: a `filter` derived from `filter_map` allocates a `Maybe` per surviving element, and a two-pass `partition` applies the caller's predicate twice per element. Retires the downstream census (ten hand-rolled `find_*` in `game/cmd.sprout`, plus `any_of` in `grimward/mods.sprout`, which was this prelude function written out by hand).
- [ ] `P3` Deferred non-goals from `docs/collection-combinators-v0.md` §7 and `docs/filterable-v0.md` §7. (a) **`length` / `is_empty` over `Foldable`** — a fold-derived version is O(n) *even on `Vec`*, where `vec_length` is O(1), so shipping them as free functions would put two spellings of the same question at different complexities next to each other in one prelude. Doing it right means `Foldable` class methods with per-instance overrides (Haskell's design for `length`/`null`), which Sprout cannot soften with default method bodies — `parse_class_body` collects signatures only — so both become mandatory for every future instance. Also note `list_length` is **private** (`prelude.sprout`), so List has no public length at all today and `count(\_ -> true, xs)` is the O(n) stand-in. (b) ~~**A short-circuiting `Foldable` primitive**~~ **DONE 2026-08-29 — `fold_while`** (`docs/fold-while-v0.md`, spec §8.5). `Foldable` gained a second mandatory method `fold_while_values(step, cond, init, xs)`, where `cond` is a *continue* condition in the `take_while` sense; `any`/`all`/`find`/`find_map`/`member` are rebuilt on it and now stop at the deciding element, and it changed no signature exactly as predicted here. `count` stays on `fold` (it must see every element). Chose the predicate form over itertools/Clojure's `Continue`/`Done` sentinel because Sprout boxes every constructor (`IRMakeCtor` → `sprout_alloc_obj`, nullary included), so a sentinel costs one allocation per element where Rust's `ControlFlow` costs nothing — and "unbox it in codegen" is monomorphization, not a peephole (`cpr_result` is false for type-variable results by design). The sentinel stays derivable from the predicate form at cost only to its callers; sketch in the design doc §6. Also deleted `vec_any_indexed`/`vec_all_indexed` — `vec_any`/`vec_all` are now `vec_fold_while` specialisations. (c) **`position` / `index_of`**, and `take`/`drop`/`zip` (List-shaped, not `Foldable`-derivable). (d) **`Dict`/`Set` instances**, gated on those types getting `Functor`/`Foldable` first. (e) An **effectful predicate** (`a -> Bool !{e}`), the same B2 blocker as `class Each`.
...
- [ ] `P2` **Retire five private re-implementations of `split`.** `stdlib.string.split(raw, sep)`
  landed 2026-09-05 with `stdlib.fs.path` (M1), because the path module needed it and the tree had
  no general splitter — only `split_once`. It has five hand-rolled equivalents, each private to its
  module: `template.split_on:81`, `url.split_amp:70`, `ast_to_ir.split_on_comma:1198`,
  `http_server.split_header_lines:186` and `prelude.split_on_char:1692`. Migrating them is a
  mechanical sweep, deliberately not bundled with the change that added the function
  (Collaboration Rule 2). Two things to check when doing it: the new one is **O(bytes × |sep|)**
  via `str_slice_bytes` + `str_starts_with_at_byte`, where the obvious `split_once`-on-the-remainder
  recursion is quadratic — so at least `split_header_lines` and `split_amp` may get faster, not just
  shorter. And it **keeps** empty segments (`split("/a//b/", "/")` is `["", "a", "", "b", ""]`),
  so a caller that relied on its private version dropping them needs a `filter` at the call site;
  `path.split` is the worked example.

- [ ] `P3` **No `fst` / `snd` in the prelude.** Surfaced 2026-09-05: `fs.read_dir` returns
  `List (String, Entry)`, and reading one side of a pair inside a lambda has no spelling —
  `\ (a, b) -> …` is a **two-parameter** lambda in Sprout, not tuple destructuring, so
  `count(\ (_, e) -> entry_is_dir(e), entries)` fails to typecheck with
  `Bool vs Entry -> Bool`. The workaround is a named function with a tuple pattern per projection
  (`pair_kind`, `pair_name` in `tests/stdlib/test_fs.spr`), which is three lines to say `snd`.
  Tuples are otherwise first-class — they are constructible, patternable, and appear in stdlib
  signatures (`url.parse_query -> Vec (String, String)`, `string.split_once -> Maybe (String, String)`)
  — so the accessors are the gap, not the type. Worth deciding alongside it whether a
  destructuring lambda (`\(a, b) -> …` as one tuple parameter) is wanted, since that is the reason
  the accessors are rarely missed in languages that have it; note the syntax currently *parses* and
  means something else, so it would be a change of meaning rather than new syntax.
...
- [ ] `P2` Import diagnostics carry no source position. `module_loader.ImportSpec` is `ImportAlias String String` / `ImportSelective String (List String)` with no `SourcePos`, so the two checks in `bundler.make_bundle_or_err` (unexported name, name bound twice) report `no_pos()` and name the file in the message text instead of pointing at the import line. Threading a pos through `parse_import_line` reaches four consumers (`bundler.spec_to_path`/`apply_one_import`, `lsp_driver`, `analysis_service_driver`) and would also let the LSP put a squiggle on the offending name. Note positions from an IMPORTED file do not currently render either (`issue_pos_for_entry` blanks any pos outside the entry file), so this wants both halves to be useful.
- [ ] `P3` `stdlib.collections` is a 7-line module exporting one function (`vec_singleton`), yet ~20 corpus files imported names from it — `Vec`, `Dict`, `vec_get_or`, `foldable_to_vec` — that all live in the prelude. The import diagnostics landed with those entries deleted, which leaves the module all but unreferenced — and note `vec_singleton` is itself a DUPLICATE of `prelude.sprout:286`, so no import line in the tree binds anything from the module. Decide what it is for: grow it into the real collections surface and move the prelude's `vec_*` there, or fold it away. **Do not simply delete it**: the duplicate name is load-bearing as a fixture. `module_loader.sprout:303` picks own_pairs by declared-name selection rather than key-exclusion *specifically* because "collections.sprout re-declares vec_singleton", and it is the only in-repo case exercising that path; `tests/stdlib/compiler/test_compiler.spr:123-142` is an explicit regression that `collections.vec_singleton` stayed reachable after being silently dropped in favour of the prelude's once before. Deleting the module removes the fixture for a bug that has actually happened.
- [ ] `P3` `export extern fn` is dead syntax, and the tree has 8 of them — `stdlib/net.sprout` (7: `tcp_listen`, `tcp_accept`, `tcp_write`, `tcp_connect`, `tcp_read_some`, `tcp_close`, `tcp_close_listener`) and `stdlib/task.sprout:64` (`task_yield`). An extern never enters a module's exported set (spec-v0 §"Externs are outside the module system") and `export` on one is "a parsed-and-discarded no-op" (`parser.sprout:2227`), so each of those reads as a deliberate visibility decision that does nothing. Same defect class as the stale import names the import diagnostics now reject: syntax stating an intent the compiler discards. Decide between rejecting `export extern fn` at parse time (a diagnostics change — it would make the dead intent loud, and is the option consistent with the import rules) and simply deleting the keyword at the 8 sites (silent, and leaves the next author free to write it again). A lint rule is the third option if a hard error is judged too strict.
...
- [ ] `P2` Organize OS-level primitives into a coherent `stdlib.os` (or split `stdlib.io` / `stdlib.env`) package: `read_file`, `write_file`, `env_get`, `argv_get` are currently in `prelude.sprout` (globally available) but logically belong in a namespaced OS/IO module. Migration requires updating all stdlib and example callers, rebuilding stage-0/stage-1, and deciding whether to keep prelude re-exports for backwards compat. Also a home for future additions: `make_temp_file`. Prerequisite: `module prelude` header landed (to avoid symbol collision fallout). **Narrowed 2026-09-05:** `list_dir`, `path_join`, `path_exists` and `delete_file` are no longer on this list — they landed as `stdlib.fs` / `stdlib.fs.path` (see below), which also settles where the namespace goes: `stdlib.fs`, not `stdlib.os`. What remains of this item is relocating `read_file`/`write_file`/`env_get`/`argv_get` out of the prelude.

- [x] `P1` **A real filesystem surface — `stdlib.fs` + `stdlib.fs.path`. LANDED 2026-09-05** (TUI/IDE arc M1). Design, prior-art survey and the rejected alternatives are in `docs/stdlib-fs-v0.md`. `stdlib/fs.sprout` grew from 36 lines (`read_text`/`write_text` only) to listing, metadata, binary I/O and mutation with a **matchable `FsError`** — the old `Err` was a String its own comment documented as unparseable, so a caller could not tell "missing" from "denied". Seven builtins (`fs_list_dir`, `fs_stat_path`, `fs_read_bytes`, `fs_write_bytes`, `fs_make_dir`, `fs_remove`, `fs_rename`), each with a rule-4 entry in `runtime/APPROVED_BUILTINS`; `read_dir`, `make_dir_all` and `remove_dir_all` are **Sprout over those primitives**, not C, per rule 2. `stdlib/fs/path.sprout` is pure and builtin-free. Three findings worth keeping. (a) **`stat` must FOLLOW symlinks.** The first draft made `stat` mean `lstat` and shipped no following variant; `/tmp` is a symlink to `/private/tmp` on macOS, so `is_dir("/tmp")` answered false and every `make_dir_all` under `/tmp` failed. Rust and Go both expose both and both name the *following* one `stat`. Caught by the tests, not by review — "the more conservative primitive" is not the same as "the right default". (b) **`write_bytes` had to ship with `read_bytes`.** Sprout could send arbitrary bytes over a socket but not put them in a file, so the binary reader would have arrived without its pair *and* untestable on the case that motivates it, nothing in the language being able to create a non-UTF-8 file. (c) **`Entry` carries no name** — the runtime filling one would have put a second implementation of POSIX `basename(3)` beside `path.basename` to drift apart; `read_dir` returns `List (String, Entry)` instead.
...
- [ ] `P2` **The review gate silently stops enforcing after 5 blocks, and cannot be re-armed from inside a session (2026-09-07).** `scripts/review_gate.py:23` sets `MAX_BLOCKS = 5`; past it the Stop hook prints `[review-gate] block cap reached — passing through` and returns 0 for the rest of the session. Observed at `blocks: 17`, 18 distinct tree states seen — i.e. roughly a dozen turns ended with no checklist, and the one-line pass-through message is easy to miss in a long session. The cap itself is correct and must stay: a Stop hook is the only thing that can gate "the work is finished", which is exactly what makes an uncapped one able to deadlock a session. What is missing is (a) recovery — resetting `blocks` means writing `$GIT_DIR/claude-review-gate/<session>.json`, which an agent editing the guard on its own turn-ending is rightly blocked from doing, so it needs a `just` recipe a human can run; and (b) escalation — the pass-through should say how many blocks were skipped and how to re-arm, rather than reading like a routine notice. Note when fixing: deleting the state file is the wrong reset, because the next Stop re-records the baseline from the *current* tree and thereby blesses everything already dirty; keep `baseline`, clear `blocks` and `seen`.
...
- [x] `P2` **FIXED 2026-08-23 — for three of the four. Smoke shapes were assigned `no_prelude`
  by design and never marked; their goldens were ~12.5k lines each for ≤7 lines of source.**
  `docs/prelude-scope-v0.md` §8 dispositioned `tests/smoke_shapes/*.spr` as "`no_prelude` —
  keeps goldens tiny" and predicted outright "The 4 smoke shapes stay small because they opt
  out." **0 of 10** carried the directive. `01_noparam`, `02_adtmatch`, `03_strconcat` and
  `05_lambdaparams` were the small ones (66/110/81/99 lines); the other 6 already had headers or
  imports and were already ~12.4k. Those four had grown **356 → 49,981 lines**; `01_noparam.spr`
  is two lines of source and its golden went 66 → 12,471 (189×).

  **Not an accepted trade — an unrecorded gap.** `prelude-scope-v0.md` §0 lists four places
  implementation corrected the design and this was not among them, so it was never revisited.
  The growth also sat outside that design's own budget, which accounted for "+88k lines … from
  the 7 examples" while *excluding* the smoke shapes on the assumption they would opt out.

  **Fixed:** `01_noparam`, `02_adtmatch` and `05_lambdaparams` now carry the directive; their
  goldens are 74 / 117 / 107 lines, recovering **37,191 lines**. Each links and runs to the same
  output as before (`42`, `0`, `7`), so the marks changed golden size and nothing else.

  > **`03_strconcat` CANNOT opt out, and the entry's claim that it could was wrong.** "Verified
  > 2026-08-22 by marking all four in a scratch copy … 64/108/79/97 lines" does not reproduce
  > against master's compiler: `no_prelude` on that file fails at check with
  > `No Semigroup instance for String in function cat`. `++` is not a call to the `str_concat`
  > extern — it is `Semigroup.append` dispatch, lowering to `__tc_Semigroup_String_append`, whose
  > body then calls `str_concat`. The floor (`docs/no-prelude-core-v0.md`) carries the prelude's
  > primitive-only **externs** and no types, classes or instances, so `instance Semigroup String`
  > is not in it. This is the floor behaving as designed, not a gap in it.
  >
  > It should stay opted in. That operator → instance → extern chain is what the shape covers,
  > and the smoke-shapes gate's `str_concat(ptr null,` assertion is a NEGATIVE one that passes
  > vacuously on IR never reaching `str_concat` — so rewriting the body as a direct
  > `str_concat(a, b)` to buy the opt-out would keep the assertion's text while dropping the
  > dispatch layer it watches. The reasoning is recorded in the fixture itself. Retrying the mark
  > fails loudly (exit 1, positioned diagnostic; `ir-golden-diff` reports `REGRESSION`), so this
  > cannot be re-broken silently.

  Recovering 37,191 lines matters beyond tidiness: `scripts/ir_golden_diff.sh:55` truncates each
  file's diff at `head -40`, so against a 12.5k-line golden the report was at most a 0.3% sample
  — the exact hazard DoD #12 warns about. The three new goldens are short enough to read whole.
...
- [x] `P1` **Dot access "Scope B": postfix `.field` on any expression is a LEX ERROR.
  FIXED 2026-08-18** (`ed7b2c93`, "parser: postfix `.field` on any expression (dot access
  Scope B)"); entry closed 2026-08-24, the fix having been found already on `master` during a
  backlog re-scan.
  **Resolution — exactly the fix shape proposed below.** `lexer.single_symbol`
  (`lexer.sprout:91`) now carries `.` in its symbol set, so a dot that does not continue an
  identifier run becomes a standalone token instead of `Unexpected character .`; the parser's
  unified postfix loop gained `is_field_postfix_at` / `parse_field_postfix` / `fold_field_chain`
  (`parser.sprout:1132-1150`), building the same nested `ast.GetFieldExpr` the Scope A
  resolution rule builds, and interleaving freely with call and `with`-update postfix.
  `.` stays an ident-CONTINUE character, so `mk_line().from.x` lexes as `.` followed by the
  single token `from.x` — `fold_field_chain` splits the remainder left-to-right. Precedence is
  preserved by ordering, not by special cases: `try_multi_char_symbol` claims `..` and the
  digit-run path claims a float's dot, both before the symbol fallback.
  Regression: `tests/stdlib/test_postfix_field_access.spr`. Original report follows.
...
- [x] `P1` **Field access on an IMPORTED (or any module-level) binding: `v_zero.x` →
  `Unknown variable: v_zero.x`. FIXED 2026-08-18** (`b9131b15`, "bundler: apply the head-first
  dot rule to module-level bindings"); entry closed 2026-08-24, the fix having been found
  already on `master` during a backlog re-scan.
  **Resolution.** `bundler.dotted_value_field_access` applies the §4.3 head-first rule at the
  phase that still knows the head's canonical name, folding the remainder into the same
  `GetFieldExpr` chain `infer.infer_var_or_field` builds — and splitting it with the same
  `string.split_once`, so the two phases cannot disagree about where a chain divides. That
  disagreement was the bug. It is consulted only AFTER module qualification declines, which is
  precisely the additive route the precedence hazard below asks for: every name it rewrites is
  one that fails to compile today, and the claimed path (`string.trim`) keeps its shadowing
  semantics untouched. Locals and parameters are deliberately left to infer's copy. Evidence
  nothing moved: `just ir-golden-diff` 58 files / 0 differences, seed fixed point at iteration
  2. Regression `tests/stdlib/test_dot_access_module_level.spr` (13 cases, RED-verified at
  `25:26: Unknown variable: origin.x`) with fixture `testsupport/rec_fixture.sprout` — covering
  the imported head, the same-module head, multi-segment chains, argument position and inline
  function-typed field calls, plus three guards pinning parameter heads, local `let` heads and
  module qualification, since the risk here is over-claiming a name. `docs/records-v0.md` and
  `docs/spec-v0.md` §5.6.3 were updated in the same commit.
  Original report follows. Repro: module `fixture.geom` exports `let v_zero = Vec2(x = 0, y = 7)`; an
...
- [x] `P2` Investigate trivial pattern-match-and-return codegen bug — duplicate `entry:` LLVM.
  **CLOSED 2026-08-24 — OBSOLETE.** The defect was specific to the legacy direct backend, and
  that backend is gone (`stdlib/compiler/codegen.sprout`, deleted in M3.2), so the emit path the
  entry asks to bisect no longer exists. This is already recorded elsewhere in this file: the
  PR 2.5 follow-up under "Current Priorities" (the `CtorInfo` record refactor) states that the
  trivial-accessor bug "did NOT recur — it was `codegen.sprout`-specific and that backend is
  retired", having converted 15 destructure sites of exactly the shape that used to trigger it.
  The workaround the entry describes is therefore also retired: inline destructuring at those
  call sites has since been replaced by named `sprout_ir.CtorInfo` field reads, pinned by
  `tests/stdlib/test_ir_ctor_info.spr`. Original text follows. Adding three tiny accessor helpers to `stdlib/compiler/ast_to_ir.sprout` (e.g. `fn ctor_tag(entry: (Int, Int, Int)) -> Int = match entry with | (t, _, _) -> t` and two similar) caused the legacy codegen (`stdlib/compiler/codegen.sprout`) to emit LLVM with a duplicate `entry:` block label deep in the stage-2 output. Both `just test` and `just refresh-seed` failed with `opt: ... error: unable to create block named 'entry'` at line ~130912 / ~226512 of the lowered file. The exact same shape exists elsewhere in the codebase (`codegen.sprout:1566`, `prelude.sprout:425`), so the trigger condition is subtle — possibly related to the helpers being mutually adjacent, all having tuple-of-Int as param type, or being called in close sequence. **Workaround in place**: PR 2.3 / commit `3049854` reverted those three accessors back to inline `match entry with | (tag, actual_ar, max_ar) -> ...` destructure at the two call sites; project memory `project_trivial_accessor_codegen_bug` records the empirical signature so future agents don't reintroduce. **The investigation**: build a minimal repro (likely 5-10 lines of Sprout source emitting duplicate `entry:` in lowering), bisect into `codegen.sprout` to find the offending emit path, fix. Once fixed, the deferred refactor of `ast_to_ir.sprout` ctor-table accessors can land.
...
- [ ] `P2` **A function PARAMETER named `entry` emits invalid IR.** Surfaced 2026-09-05 while
  ...
  **Root cause.** `ir_lowering.lower_params` (`stdlib/compiler/ir_lowering.sprout:440-444`) emits
  each parameter as `i64 %<source name>`, verbatim and unmangled, while `ast_to_ir` names every
  function's first basic block `"entry"` (`ast_to_ir.sprout:5907`). LLVM's textual IR keeps local
  values and block labels in **one** namespace, so `%entry` and `entry:` collide.

  **Measured scope** (`--emit-ir` + `opt --passes=verify` on a three-function probe): only
  *parameters*. A `match` binding and a `let` binding of the same name are both fine, because those
  lower to `%t$N` temporaries — which is also why `remove_dir_all` in `stdlib/fs.sprout` can still
  write `Ok entry`. Parameters are the only user-controlled names that reach the value namespace
...
- [ ] `P3` **DCE keeps an unreachable stdlib function in one corpus file.** Observed 2026-09-05
  ...
  `define @stdlib.string.split` and `@stdlib.string.split_go` (~175 lines) even though **nothing
  calls them** there — the only `call` to `split` in that file is `split`'s own call to its helper.
  The other 13 corpus files that import `stdlib.string` correctly prune both. Harmless today (dead
  IR, not wrong IR), filed because a per-file inconsistency in DCE is the kind of thing that grows.

  **Two plausible explanations, both REFUTED by probe** — recorded so the next person does not
  re-derive them:
  - *Name-prefix reachability* (`split` kept because `split_once` is reachable, and `sentry_api` is
    the only one of the five checked that contains `split_once`). A probe calling **only**
    `string.split_once` emits `split_once` and **no** `split`. The correlation was perfect and the
    mechanism was still wrong.
  - *The import set*. A probe with `sentry_api`'s exact four imports, calling only `split_once`,
    also emits no `split`.

  So the trigger is something in `sentry_api`'s **body**, not its imports and not the name. Next
  step for whoever picks it up: bisect the example's body rather than its header.
...
- [~] `P2` **`assert_eq` on `Double` — the silent no-op is GONE (re-measured 2026-08-20); what remains is the missing `instance Eq Double`.** As filed (2026-07-10) this said a `Double` `assert_eq` neither prints PASS/FAIL nor increments the counter, so any such test was a false green. That is no longer true: the same program is now rejected at compile time with a located `No instance of Eq for Double`, verified with a deliberately-unequal pair (`assert_eq(state, …, 1.5, 2.5)`) — the exact regression test this entry asked for, which can no longer be written in Sprout because it does not typecheck. **The false-green risk is closed** (most likely by the superclass/unresolved-dict work of 2026-07-27/29 further up this section, which turned null-filled dicts into compile errors and loud panics). **What is still open is narrower and is a stdlib gap, not a dispatch bug:** `stdlib/prelude.sprout` has `instance ToString Double` (`:863`) but **no `instance Eq Double` and no `instance Ord Double`**, so a `Double` cannot be used with any `where Eq a` / `where Ord a` function — `assert_eq`, `check_eq`, `list_contains`, sorting. The entry's own workaround explains why this went unnoticed: `assert_true(state, label, x == y)` works because `==` on `Double` is the builtin operator path, which never consults the `Eq` class. Adding the two instances is a prelude change needing a decision on IEEE semantics first (whether `Eq Double` is IEEE `==`, which makes `NaN /= NaN` and both zeros equal, or a total order — see the `-0.0`/NaN notes in §3) JSON Support). Found during digit-recognizer Phase B kernel work.
...
- [x] `P2` **`String`→`Double` parsing. CLOSED 2026-08-20 as written — the function exists; its *accuracy* gaps are tracked in §3) JSON Support.** `parse_double : String -> Maybe Double` landed at `stdlib/prelude.sprout:1452` (total, mirroring `parse_int`, with `tests/stdlib/test_parse_double.spr` covering integer/decimal/scientific/malformed forms), so this entry's premise ("no equivalent exists") went stale and was still being read as an open gap on 2026-08-20. What is genuinely open is not its existence but its fidelity: it rejects a `+`-signed exponent, cannot represent `-0.0`, and is 1 ULP off below 1.0. Those are filed with measurements as sub-items (a), (c) and (d) of the `Double`-through-JSON entry in **§3) JSON Support** — do not re-file them here. Original entry follows. No `parse_double : String -> Maybe Double` (or equivalent) exists — `Double` currently has no textual round-trip in the direction a program needs for reading numeric input/config/JSON values. Mirror the existing total `parse_int : String -> Maybe Int` (`stdlib/prelude.sprout`). Surfaced as a remaining float gap alongside the `to_int`/`fptosi` item above when `Double` landed 2026-07-06.
...
- [~] `P1` **A user-defined `where`-constrained function used as a first-class VALUE compiles, links, and dies at runtime: its dictionary parameters leak into the closure's arity (reported 2026-08-20 from `uncharted-suns`, reproduced here on `c90bf97a`). FIXED 2026-08-20 wherever the dictionary is readable at the mention: an argument whose receiving parameter type FIXES the constraint variable, or any position inside a `where`-constrained caller (which forwards its own). The rest is tracked as its own item below.**
  **The boundary is the receiving slot, not the syntax** — worth stating precisely, because "argument position is fixed" is the wrong summary and would send a later reader past a case that still fails. `apply1(label, 3)` works because `f: Int -> String` fixes `a = Int`; `Just(label)` consumed as `Maybe (Color -> String)` is *also* argument position and still fails, because `Just`'s parameter is a bare tyvar and `push_down_arg_slots` has nothing to push. Record fields, list and tuple elements fail for the same reason. Verified directly, not inferred.
  **Fix (infer.sprout, `eta_expand_constrained_arg` + `constrained_fn_eta`):** a bare mention of a constrained function is rewritten into the eta-lambda the programmer could have written — `label` becomes `\__eta_x0 -> label(__eta_x0)`. Nothing new resolves a dictionary: a *call* already gets one (infer prepends `TDict`, resolve fills it, lowering appends the witness), so restoring the call shape is enough. Arity comes from `fn_declared_arity`, not from counting arrows, because a zero-parameter function returning a function has the same `a -> b` scheme as a one-parameter one; class methods are excluded via the `@class:` marker (they already work through `TMethodRef`), and a local shadowing the name drops its `@arity:` marker in `bind_local_scheme`, so shadowing falls through unchanged.
  **The placement is the whole content of the fix, and it is not where this entry predicted.** The rewrite must happen in `infer_arg_slots`, *before* an argument is classified — that match is where Sprout decides an argument's fate, and it decides syntactically: a `LambdaExpr` is held back behind a fresh slot and later typed against the callee's declared parameter (`push_down_arg_slots`), while everything else is inferred eagerly with no expected type. Built one level deeper, inside `infer_var`, the synthesized lambda lands on the eager branch, where dictionary resolution has only the constrained function's own signature to go on and guesses from its return type — `apply1(label, 3)` resolved `ToString String`, caught by `verify_dispatch`. Same code, one call frame earlier, works.
  Test `tests/stdlib/test_constrained_fn_as_value.spr` — 11 cases across three hidden-arity classes (`ToString` 1 slot, `Enum` 2, `MyOrd` = own + inherited `MyEq`) so a one-slot implementation fails rather than passes, plus a two-constraint case pinning dictionary order, `list_map` for a prelude HOF, the `where`-constrained-caller `let` at three types, and the free-tyvar dead dictionary staying inert. RED before the change with the reported arity abort.
  Original report follows. `fn label(x: a) -> String where ToString a = to_string(x)` applied directly works; the moment `label` is *referenced* rather than applied, the program aborts with `runtime error: this function value expects 2 arguments, got 1 — if it returns a function, apply the rest in a separate call, as f(…)(…)`. The user wrote a one-argument function and the message tells them it takes two: the extra argument is the injected `ToString` dictionary, which is an implementation detail the source cannot see, name, or supply. **The count tracks the dictionary shape, which confirms the mechanism** — one method (`ToString`) → arity 2; `Enum` (`ordinal` + `from_ordinal`) → arity 3; `Ord` (its own method + the `Eq` superclass) → arity 4, i.e. one slot per method rather than one per class.
  **Reproduced in five independent shapes, all identical:** `let f = label in f(1)`; returning it (`fn pick() -> Int -> String = label`); passing it to a monomorphic HOF (`apply_it(label, 3)`); passing it to `list_map`; and passing it to a *polymorphic* HOF (`twice(f: a -> String, x: a)`). It is not specific to `list_map` or to inference failing to pin the type — the element type is concrete `Int` at every one of those call sites.
  **This is a gap in the eta-expansion path, not in dict resolution.** Class *methods* in value position already work — `list_map(to_string, [1,2,3])` and `let f = to_string in f(42)` both run correctly, and `tests/stdlib/test_eta_forwarding.spr` pins exactly that path (`try_eta_in_class` / `try_eta_forwarded_without_class` reconstruct an eta-lambda that forwards the hidden dictionary slot). What has no such path is a user `fn` carrying its own `where` clause. Likely fix: extend the same eta-reconstruction to a value-position reference to a constrained top-level `fn`, wrapping it in a lambda that closes over the resolved dicts so the closure's arity matches its source-level type. **Minimum acceptable outcome is a located compile error** — the language already refuses first-class use of compiler intrinsics (`print`, `to_double`, `double_to_bits`, `stdlib.bits`; `spec-v0.md:1620` and `:1644`), but those fail loudly at *link* time. This one type-checks, links, and reaches a user's runtime, which is strictly worse than any of them; §"Language Core and Safety" treats precise-or-loud as the standard.
  **Discrepancy to resolve before fixing — the report and this reproduction disagree about the failure message.** The `uncharted-suns` agent reported this shape aborting with the unresolved-dict poison thunk's *"compiler-soundness backstop; please report"* string (`ast_to_ir.sprout:7193`). Ten probe shapes here reached the arity panic or ran correctly, and **none** emitted a poison thunk into the IR (`grep unresolved_thunk` = 0 in every case): seven aborted with the arity message, three ran clean (the two class-method-in-value-position shapes, plus the free-tyvar one). The seven include the two that mirror `game.wire_json` directly (`je : a -> Json where Enum a`, and a return-position-only constraint like `ge`) and a superclass-carrying class. Among the three that ran clean is the free-tyvar dead-dict case (`list_map(label, [])`), which is correctly inert at exit 0 exactly as its design intends — so the legitimate never-invoked path is confirmed healthy here. **`Double` was the strongest remaining candidate and is now eliminated**, since the entry at §Compiler/Stdlib Misc recorded a forwarded `Eq`/`ToString` dict for `Double` resolving to a no-op — exactly the null-fill the poison-sink fix converts into this panic — and `wire_json` is `Double`-saturated. Probed directly: a forwarded `ToString` dict at `Double` (`label(3.14)`, and twice-forwarded) runs correctly, and the `Eq` half cannot be reached at all because there is **no `instance Eq Double`** — the call is a located compile error, not a poison. So either the poison is reached by a *further* shape not covered here, or a compiler version difference is in play. This matters because the poison entry above (`item: make the unresolved-dict SINK loud`) records that **"there is no source-level RED that INVOKES a poison"** — a real source-level poison would falsify that and is the more severe of the two bugs. **Next step: get the exact triggering expression from the reporter before choosing the fix**, since a poison-reaching shape would need the producer-side guard, not the eta path.

- [x] `P2` **FIXED 2026-08-22 — and direction (b) was never needed. What this item got wrong is the useful part.** The entry below enumerated FIVE remaining positions (a local `let … in`, `Just(label)`, a record field, a list element, a tuple element) as if each were a separate site needing its own propagation work, and concluded they needed deferred resolution — "direction (b)" — with a design note first because deferral touches the resolve/lowering split `M3b-5` settled. That framing was wrong twice over. They were not five defects but **one resolver reading the wrong place**, and it was closed by `b53526d4` (#178), a change filed against an unrelated symptom, which shipped without anyone noticing it also closed this. Neither *push the type down* (a) nor *defer the decision* (b) was ever the fix; the fix was **look in the right place**. `find_inst_in_return_type` scanned the concrete return type for any component carrying a matching `@inst` and took the first hit — landing on `String`, `label`'s OWN return type, which is why every position produced the identical `resolved to ToString String` message. `constraint_var_dict` instead walks the callee's declared type alongside the type the call instantiated it to and asks where the constraint's variable actually sits. That runs in `maybe_rewrite_constrained_fn_call`, a post-pass after `check_fn_body`'s `s2`, so unification has already filled the variable in — **which is why the position of the value never mattered and one change closed all five.**
  **Verified by bisection, not by inspection.** A ten-shape battery — the five positions, plus a lambda vs. bare-name variant, two containers deep (`Just([label])`), across a function boundary (`fn make() -> Maybe (a -> String) where ToString a`), and a distinctive user instance rendering `TAG<42>` so `Int`/`Bool` cannot pass by luck — was run against drivers linked from four historical seeds. All fail at `6956f638` (#178's parent) with the recorded message; all compile, link, run, and print correctly at `b53526d4`. `fbfacb0e` (return type as evidence) and `11fed12f` were checked too and change nothing here, so the earlier "(a) landing cleanly is evidence it is not needed for the second" reasoning was also unfounded. One control stayed green throughout — `apply_to(label, n)`, an ARGUMENT position, which `infer_arg_slots` already handled — so the battery is not uniformly red and the contrast means something.
  Pinned by `tests/stdlib/test_constrained_value_positions.spr` (18 assertions, every shape at two types and the record cases at a third), which is RED on `6956f638`. `docs/spec-v0.md` §"Type annotation" carried the same error normatively — "a position that fixes nothing (a record field, an element of a list) determines no instance" — and is corrected in the same change.
  **Two things found alongside are NOT residuals of this item, and should not be re-filed against it.** (1) A local `let … in` binding used at two types in one body is rejected (`Type mismatch: Int vs Bool`) — that is let-monomorphism, a separate question about generalization, not about dispatch. (2) `let f: Int -> String = label in …` is a parse error; per `docs/spec-v0.md` annotations on `let … in` are explicitly not yet in the language, tracked in `docs/binding-annotations-v0.md`. Original entry follows.
  ~~PARTIALLY FIXED 2026-08-20/21 — direction (a) is CLOSED: both positions that carry a declared type, an annotated top-level `let` and a function's declared return type, now push it down. What remains needs direction (b), because it has no declared type to push: a local `let … in`, and any position whose receiving slot fixes nothing (`Just(label)`, a record field, a list or tuple element).~~ `fn get_labeller() -> Int -> String = \n -> label(n)` and `let f = \m -> label(m) in f(n)` — for any `where`-constrained `label`, **in a caller with no `where` clause of its own** — are rejected with ``dispatch-verify: dictionary for `ToString a` resolved to `ToString String` but the call fixes a = Int``. **This is not about constrained functions in value position, and it predates that work:** both shapes fail identically, written out by hand, on a compiler without the fix above. The tests for that item deliberately exclude these two positions for this reason.
  **Scope, measured rather than assumed.** The same two positions *inside* a `where`-constrained caller work correctly — there the caller has a forwarded dictionary slot for the reference to close over, and nothing is guessed. Probed at three types including a user instance with distinctive output (`Int`→`7`, `Bool`→`true`, custom→`TAG<42>`), since a scan-chosen dictionary could satisfy one type by luck but not all three; return position likewise. This is worth stating because it is exactly the family `verify_dispatch` phase 1 records as **skip-and-log, not verified** — a wrong dictionary there would be silent rather than loud. It is not wrong. Both shapes are pinned in `tests/stdlib/test_constrained_fn_as_value.spr`, along with the free-tyvar dead-dictionary case (`list_map(label, [])`), which stays inert at exit 0.
  **Mechanism.** Sprout's inference is bidirectional for lambdas in exactly one place: `infer_arg_slots` holds a `LambdaExpr` *argument* back behind a fresh slot and types it against the callee's declared parameter (`push_down_arg_slots`). A lambda that is a function's return expression, or a `let` right-hand side, is not an argument to anything, so no expected type is ever pushed in. Its parameter is still a metavar when the body's constrained call resolves its dictionary, and the resolver falls back to scanning — landing on `String`, the constrained function's *return* type. `verify_dispatch` then rejects the mismatch, which is why this is a loud compile error rather than a call into the wrong instance: the soundness net is working, and the gap is upstream of it.
  **Two directions; (a) chosen and half-landed.** (a) Extend expected-type propagation to the two non-argument positions that actually have a declared type to propagate — a function's declared return type, and an annotated `let`. This is the fix that matches what a reader expects an annotation to do; the annotated-`let` half landed 2026-08-20 (below), the return-type half has not. (b) Defer dictionary resolution for a call whose constraint variable is still unresolved, so the choice happens after unification. Broader, and it touches the resolve/lowering split `M3b-5` deliberately settled — worth a design note before attempting. Not attempted, and (a) landing cleanly at the first site is evidence it is not needed for the second.
  **The diagnostic is a separate, smaller defect for whatever remains.** The `dispatch-verify` message describes a dictionary the user never wrote, in terms of a type variable from a signature they may not have looked at. A message naming the real problem — a lambda parameter whose type is not determined where it is needed — is worth having regardless of which direction lands. (The top-level `let` shapes no longer reach it: they either resolve, or get the `ambiguous typeclass binding` message written for them.)
  **The top-level `let` shapes are FIXED 2026-08-20; a function's declared RETURN type is what remains.** The mechanism was not what this entry predicted, and the correction is the useful part: `resolve_dispatch_typed_expr` — the post-pass that fills a dictionary slot inference-time injection left open — is called from `check_fn_body` (`infer.sprout:6962`) and `check_instance_methods` (`:7354`) and **from nowhere else**, so a top-level `let` initializer was the one body in the language that never got it. That, not the annotation, is why `let d: Int = make()` (an ordinary call to a return-position-constrained fn, no value-position eta anywhere) reached codegen an argument short and emitted ``internal error: under-application of 'make' reached codegen (arity 1, got 0)``. Three changes, all in `infer.sprout`:
  (1) **Run the post-pass on a `let` initializer**, with the post-`apply_let_annotation` substitution — for a `let` the annotation is the only thing that makes the return type concrete, so running it earlier would fill the slot from a guess.
  (2) **Check a lambda initializer against the annotation** (`typecheck_let_rhs` → `infer_lambda_expected`, the entry point argument push-down already uses) instead of inferring it and unifying afterwards. Haskell 2010 §4.4.1 is the rule this restores: "each use of f is treated as having the declared type. It is a static error if the same type cannot also be inferred for the defining occurrence." The test is *is a lambda after the eta rewrite*, not *was eta-rewritten*, so a hand-written `\n -> label(n)` is covered identically.
  (3) **Reject an ambiguous constrained binding** (`let_dict_ambiguity_msg`): a top-level `let` that quantifies over a type variable its initializer needs a dictionary for is a located error naming the class and the type (`ambiguous typeclass binding: … add a type annotation that fixes it`). **This one is load-bearing, and (1) is unsafe without it:** with the post-pass running, unannotated `let labeller = label` compiled to a closure carrying the `ToString String` guess and SIGSEGV'd on an Int — a *silent* miscompile where the internal error had at least been loud. `verify_dispatch` cannot catch it (the call fixes nothing concretely, so it is skip-and-log), and the TDict node cannot either: its head was committed when the node was built, so the node cannot say whether that choice was determined or guessed. Only the binding can. Haskell reaches the same place from the other side — §4.5.5 refuses to generalize a constrained binding, then §4.3.4 defaulting picks a type; Sprout has no defaulting, so it must reject.
  **The guard's first cut was too coarse, and the shape that caught it is worth keeping in mind.** "Quantifies over something AND contains a dictionary" rejects `let tag = \x -> (x, to_string(7))` — which quantifies `a`, contains a dictionary, and is perfectly well-determined, since the `7` fixes `ToString Int`. That shape compiles and runs correctly on the compiler before this change, so the coarse rule was a regression, found by probing the boundary rather than by any gate: the corpus and `uncharted-suns` were green because neither happens to contain the shape, which is evidence about them and not about the rule. The precise test asks the CALL, not the dict: a dictionary is determined by the constraint variable's instantiation at its call site, so if neither the call's arguments nor its result mentions a quantified variable, the choice is already made. Pinned by two cases in the test file, used at two types so a binding silently narrowed to keep its dictionary would fail.
  Diagnostics needed one supporting change: `types.type_to_string_renamed` renders the type with `a`, `b`, … instead of `$t2150`, which is a compiler leak in any user-facing message.
  Tests: `tests/stdlib/test_constrained_let_binding.spr` (4 cases — return-position dispatch at `Int` AND `String` from the same constrained fn, so one baked-in guess cannot satisfy both; the value-position annotated binding applied directly and through a hof) and `tests/conformance/type_error/ambiguous_constrained_let.spr` pinning the new rejection. Gates: full suite 311 suites/0 failures, `ir-golden-diff` **60 files 0 differences** (no corpus file could use these shapes, since doing so crashed), 33 fast gates, 53/53 examples, seed fixed point at iteration 2, and `uncharted-suns` (test-loam, test-game, compile-examples) green — that repo has ~499 top-level `let` declarations, and the new rejection fires on none of them.
  **Caveat on "fixed": one top-level `let` ambiguity is still uncaught, and it is a different check's gap.** The `ambiguous_class_tyvar` shape — `let s = to_str(from_int(7))` with two `Codec` instances, where the intermediate type occurs only between the two calls — reaches the LINKER with `use of undefined value '@from_int'`, no Sprout position. The binding's own type is ground (`String`), so it quantifies nothing and the guard above is silent by design; the ambiguous variable is internal to the initializer. Identical on the pre-change compiler, so it is pre-existing rather than caused here, and `tests/conformance/type_error/ambiguous_class_tyvar` shows the same shape IS rejected inside a `fn` body — so whatever catches it there is another pass that a top-level `let` does not reach, the same family as the post-pass gap this item fixed. Filed as its own `P2` under Dispatch Soundness and **fixed there 2026-08-20** — and it was the same family, literally: `verify_dispatch.collect_decl` dropped `TLetDecl` into a `_` arm, so no top-level `let` initializer had ever been dispatch-verified.
  **A function's declared RETURN type is now pushed down too — FIXED 2026-08-21, closing direction (a).** `fn pick() -> Int -> String = label` used to be rejected with the `dispatch-verify` mismatch above. `typecheck_fn_body` (`infer.sprout`, mirroring `typecheck_let_rhs`) checks a lambda body against the declared return type instead of inferring it blind and unifying at the declaration boundary afterwards; `check_fn_body`'s single `typecheck_expr` call routes through it.
  **The tyvar-mapping complication this entry predicted does not exist**, and the reason is worth keeping: `check_fn_body` instantiates the WHOLE signature once into `inst_type`, and the post-hoc check already read the expected return off it as `fn_return_type(inst_type, arity)`. So the expected type is available in the body's own variable world, and pushing down *that value* — rather than re-resolving the return `TypeExpr` — is what keeps `fn wrap(x: a) -> a -> String where ToString a = \y -> …` sharing one `a` between parameter and return. Resolving the TypeExpr separately is what would have minted a second one. Gated on a return type actually being written: with none, `fn_return_type` yields the metavar the signature was seeded with, which is not evidence.
  Test `tests/stdlib/test_constrained_fn_return_position.spr` (7 cases, RED first with the exact message above): the bare mention at `Int` and at a user type with a distinctive instance, a hand-written lambda in the same position (so the fix keys on *is a lambda*, not *was eta-expanded*), a two-slot class (`Enum`), two constraints across two parameters, and the `a`-sharing shape at two types as a regression guard. **Blast radius is wider than the `let` fix** — it changes inference for every `fn` with a declared return type whose body is a lambda — so the corpus evidence is load-bearing: seed fixed point at iteration 2 (the compiler self-hosts through the new path), and the seed diff is confined to exactly `check_fn_body` and the new `typecheck_fn_body`, no collateral.

- [x] `P2` **FIXED 2026-08-20. An ambiguous class-method chain inside a top-level `let` reached the LINKER, unlocated (found 2026-08-20 probing the ambiguity guard's boundary).** `let s = to_str(from_int(7))` with two `Codec` instances — the `show . read` ambiguity, where the intermediate type occurs only between the two calls — compiles to a call with no dictionary and fails as `use of undefined value '@from_int'`, with no Sprout source position. The identical shape inside a `fn` body IS rejected at check time: `tests/conformance/type_error/ambiguous_class_tyvar` pins that, and its header records the linker failure as the *pre-fix* behaviour. The `ambiguous typeclass binding` guard added alongside that fix does NOT cover this and is not meant to: the binding's own type is ground (`String`), so it quantifies nothing, and the undetermined variable is internal to the initializer. Pre-existing, verified identical on the pre-change compiler.
  **The cause was NOT the call-site list this entry guessed, and the correction widens the item.** `verify_dispatch.verify_program` already runs over the whole program, not per-`fn`. The gap was its own decl walker: `collect_decl` (`verify_dispatch.sprout:546`) matched `TFnDecl` and `TInstanceDecl` and sent everything else to a `_ -> Nil`, which swallowed `TLetDecl`. So this was never one ambiguous chain escaping — **no top-level `let` initializer had ever been dispatch-verified at all**, and the linker leak was one symptom of a whole body class sitting outside the soundness net. Structurally the same omission as the `resolve_dispatch_typed_expr` gap above (a decl-level walker that enumerates `fn` and instance methods and forgets that a `let` also has a body), in a second, independent pass — which suggests the omission is in how the decl list is habitually walked rather than in either pass.
  **Fix:** add `| typed_ast.TLetDecl _ body -> collect_expr(body, sigs, dict_empty())` (no params, so nothing shadows a callee name), and replace the `_` with explicit `TExternFnDecl` / `TPassThrough` arms so a future `TypedDecl` carrying an expression fails to compile here instead of silently skipping verification — the wildcard is what hid this.
  **The arm does real work, measured rather than assumed.** `SPROUT_VERIFY_DISPATCH_STATS` on `tests/stdlib/test_constrained_let_binding.spr` goes `verified=119 skipped=134` → `verified=123 skipped=134`, `mismatched=0` both ways: four newly-verified call sites, exactly the file's four top-level `let` initializers, all of which agree with what already compiled. Without that delta a passing suite would be indistinguishable from an arm that never fires. The same measurement downstream is the one that matters, since switching a verifier on for a body class it has never seen can reject working code and a green suite would only prove the corpus lacks the shape: `uncharted-suns` has **659 top-level `let` declarations across 141 files**, and on `game/app.sprout` alone the stats go `verified=1833` → `1945` with `skipped` unchanged at 140 and `mismatched=0` — 112 newly-verified call sites in real downstream code, all agreeing. Its test-loam / test-game / compile-examples gates stay green.
  **The acceptance criterion above was wrong on one point.** "Extend the conformance fixture rather than adding a separate one" cannot work: `_test-reject` greps the compiler's output for the expected substring, and both forms emit the same message, so a `let` binding added to `ambiguous_class_tyvar.spr` would be satisfied by the `fn`-body error alone and could never go RED. It needs its own fixture, `ambiguous_class_tyvar_top_level_let.{spr,err}`, whose header says so. Unit twin in `tests/stdlib/compiler/test_verify_dispatch.spr`: the *same* hand-built `amb_call`, wrapped in a `TLetDecl` instead of a `TFnDecl`, so only the declaration wrapper varies.

- [x] `P1` **FIXED 2026-08-21** — "infer: a generalized type variable needs a `where` constraint to supply its dictionary". The fix is `fn_dict_coverage_msg`, a declaration gate in `check_fn_body` (`infer.sprout`) that runs at generalization: subtract the variables the declared `where` clause covers from the ones the declaration generalizes, and reject if any dictionary in the body is selected for what remains. Two diagnostics — `add \`where C v\` to its signature` when the variable is in a parameter, `appears in no parameter` when it is not, because a `where` clause cannot help in the second case. Fixtures `uncovered_dict_param_position`, `uncovered_dict_no_parameter`, `uncovered_dict_wrong_class`; accepted-case suite `tests/stdlib/test_uncovered_dict_constraint.spr`.

  **Three things this item got wrong, recorded because each cost a rebuild.** (1) **The scope was not "return position".** `fn relabel(x: a) = label(x)` — the constraint variable in a *parameter* — SIGSEGVs identically. The real split is that every CLASS-METHOD shape was already rejected (verify_dispatch) and every constrained-USER-FUNCTION shape crashed; position had nothing to do with it. (2) **The call-site fix over-rejects.** `verify_dispatch.check_one`'s "concrete dict head + still-polymorphic truth" is exactly the guess, and spiking it to an error rejects both bug shapes correctly — but also `list_map(label, [])` and `vec_sort(vec_empty())` in three existing suites, whose dead dictionaries are deliberately inert. Only a check at generalization separates them, because "does the enclosing declaration generalize this variable" is the discriminator and `main` generalizes nothing. (3) **Attribution cannot be read off the call.** Deciding *which* variable a dictionary is for by scanning argument types fails on the prelude twice over: `merge_runs`'s `where Ord k` over `List (k, Int, a)` mentions the payload `a`, and `fold`'s `where Foldable f` sits behind an `init: b` that is the first variable-typed argument. The subject has to come from the callee's §13.3(B) Scheme constraint field matched against the callee reference's instantiated type (`uncovered_dict_subject`); the TDict node cannot answer it, since its head is the guess itself.

  **That residual is FIXED 2026-08-22** — "infer: an instance method's dictionary needs the same coverage as a fn's". The prediction above was accurate: the gate is a pure function of its inputs, and wiring it in was plumbing `prog_to_fresh`, the INSTANCE-level constraints, and `env_ftv` down through `check_instance_methods` → `check_instance_method` → `instance_method_checked` → `finish_instance_method`, where it sits beside `class_method_mode_error` as the twin of `check_fn_body`'s call site. The subtraction is shared with the `fn` gate (`uncovered_generalized_vars`) so the two cannot drift.
  **The remedy differs, and that is the whole reason it needed its own message.** A method carries no `where` clause: it is a parse error on the class signature *and* on the instance implementation (both probed). So `add \`where C v\`` — what the `fn` gate says — is unwritable here. The only place a constraint can go is the **instance head**, which does forward a dictionary (`instance Renderer (Pair a) where ToString a`, verified at two element types). The message says that instead.
  Rejecting is still right despite there being no general remedy: the class signature promises `for all a` and a body needing `ToString a` cannot honour it, so the program is unsound rather than merely unsupported. A method whose own signature needs a constraint separate from the class variable remains inexpressible — that is the tracked parser gap above (`P0` method-level constraints / `Traversable`), which this does not touch.
  Fixtures `uncovered_dict_instance_method` (parameter position) and `uncovered_dict_instance_method_return` (return position), both RED first; accepted-case suite `tests/stdlib/test_instance_method_dict_coverage.spr`. Over-rejection evidence, which no gate provides on its own: the compiler reached its bootstrap fixed point at iteration 2 under the new check (its own source is instance-dense), 52/52 examples, and `uncharted-suns` `test-loam`/`test-game`/`compile-examples` all green.

  **A residual first recorded here was WRONG and is retracted:** `let f = label in f(x)` inside a `where`-less generic function was claimed to escape the walk. It does not — it is rejected. A bare mention of a constrained function is eta-expanded, so the constrained CALL appears inside the `let` initializer where the walk reaches it; the local `f` never has to be attributable. The same property makes shadowing safe in the other direction (a local callee resolves to no constrained scheme, so nothing is attributed to it), which `test_uncovered_dict_constraint.spr` now pins for a `let` binder and a lambda parameter.

  Original report follows. **A constrained function returned from a fn with NO declared return type compiles clean and SIGSEGVs (found 2026-08-21 while landing the return-type push-down; pre-existing, verified identical on the pre-change compiler).** `fn pick_unannotated() = label` for a `where`-constrained `label`, with no return type and no `where` clause on the enclosing fn, emits IR at exit 0 and dies at runtime in `print_str` — the closure's arity does not match the type it advertises, the original `uncharted-suns` failure mode. It is the last shape of that family still reaching a user's runtime: every sibling is now either resolved (a receiving parameter that fixes the variable, a forwarding `where` clause, an annotated `let`, a declared return type) or a located compile error (`Just(label)`, a record field). **`spec-v0.md` claimed this was rejected and it was not**, which is why it survived — the claim has been corrected to describe the slot rather than the syntax. **Direction:** the same reasoning as the `ambiguous typeclass binding` rejection for an unannotated top-level `let` — nothing determines the instance, and unlike a `fn` with a `where` clause there is no parameter to receive one per call, so rejecting is the only sound answer. Needs the same care that rejection needed: the adjacent-legitimate case (a fn whose inferred return type *is* fixed by its body, e.g. returning a partially-applied constrained call at a concrete type) must be probed by hand on the pre-change compiler, since no gate would catch over-rejection. Acceptance: located compile error naming the class, plus a conformance fixture and an accepted-case test in `tests/stdlib/test_constrained_fn_return_position.spr`.

- [ ] `P2` **The uncovered-dictionary diagnostic prints a compiler-internal placeholder and gives advice that cannot be followed (found 2026-08-22; shipped by the `P1` above).** For an UNANNOTATED parameter the message names the synthesized tyvar:

  ```sprout
  fn label(x: a) -> String where ToString a = to_string(x)
  fn relabel(n) = label(n)
  ```
  ```
  `main.relabel` needs a ToString instance for type variable `_unann_n`, but declares no
  `where` constraint for it, … — add `where ToString _unann_n` to its signature
  ```

  `_unann_<param>` is generated by `collect_param_type_vars` (`infer.sprout:224`) and `param_types_from_decl` (`:406`); the compiler's own comment at `:6890` says these are "synthesized placeholders for UNANNOTATED returns/params (not written by the programmer)". So the suggested edit is not writable, and no rename of it would be either — with the parameter unannotated there is no type variable to constrain. **The real remedy is to annotate the parameter, which creates the variable, and constrain that:** `fn relabel(n: a) -> String where ToString a = label(n)` — verified to compile and dispatch at two types. The rejection itself is correct; only the message is wrong. **Fix:** when the subject variable's name carries the `_unann_` prefix, strip it and advise annotating *parameter `n`* rather than emitting the placeholder. Needs its own `tests/conformance/type_error/` fixture, and its message substring must differ from both existing `uncovered_dict_*` fixtures, since `_test-reject` matches by substring. Error-path only, so golden IR should not move, but it is a `stdlib/compiler/` edit and pays the full seed cascade.

- [x] `P2` **FIXED 2026-08-21** — "infer: resolve a return-only constraint by locating its variable, not by scanning". The declaration now compiles and dispatches **per call**: `pick()(5)` and `pick()(true)` select different instances. Test suite `tests/stdlib/test_return_only_constraint.spr`.

  **This did NOT need direction (b), and the item said it did.** The deferral already existed — `maybe_rewrite_constrained_fn_call` is a post-pass running after `check_fn_body`'s `s2`, written for exactly this case. What failed was locating the variable: the repair called `find_inst_in_return_type`, which scans the concrete return type for any component carrying a matching `@inst`, and `concrete_type_str` returns `Nothing` for a `TFunc` while `find_inst_in_return_type_children` descends only into `TApp` and `TTuple`. A FUNCTION-typed return was therefore never looked into at all, so no dictionary was injected and `verify_dispatch` reported the constraint as missing — rendering it as `No instance of ToString for Int` because *its* `build_theta_ret` had done the structural match and knew `a = Int`. The false message was the second pass correctly describing what the first pass failed to resolve.

  **Adding a `TFunc` case to the scan would have been the wrong fix.** For `Int -> String` under `ToString` both halves have instances, so a first-match scan picks one arbitrarily — the same mis-resolution that reading a constrained function's own return type used to cause, and it would have traded a loud false error for a silent wrong dispatch. The fix instead uses `find_class_var_in_type`, the structural walker that **class-method** dispatch already used for this identical "class variable only in return position" problem; constrained functions had kept the older scan. That asymmetry was the whole defect. The scan is retained as a fallback, so no resolution that already worked changes — pinned by the non-function case in the suite, which resolves byte-identically before and after.

  Original report follows. **A return-only `where` constraint is rejected with a message that is false (found 2026-08-21).** `fn pick() -> a -> String where ToString a = label` is a legitimate declaration — `a` appears in the signature, so constraint-validation accepts it — but dispatch verification rejects the call `pick()(5)` with `No instance of ToString for Int`, which is untrue; `ToString Int` exists. The constraint variable is determined only by how the *result* is used, so resolution at the `pick()` call has to look past the call itself; this is the deferred-resolution work recorded as direction (b) above, and the message is the visible symptom. **This matters beyond its own wrongness:** it is the shape a user lands in after following the `appears in no parameter` advice from the uncovered-dictionary check if they reach for a `where` clause instead of a concrete annotation. Fixing the message alone is not enough — the declaration should compile. Feed into direction (b)'s design note rather than patching the text.

- [ ] `P3` **A bare class-method mention with nothing to fix its instance reports a compiler internal, not a diagnostic (found 2026-08-21).** `fn pick() = to_string` emits `ERROR: ast_to_ir: unbound variable '__eta_unresolved_ToString_to_string'` with no source position, preceded by `warning: alloc-summary pre-pass failed`. It is loud rather than silent, so it is cosmetic in severity, but the text names an internal symbol and points at no line. The sibling shapes all produce proper located diagnostics: `fn f(x: a) = to_string(x)` reports the ambiguous-type-variable error, and the constrained-user-function equivalents are reported by the uncovered-dictionary check. The eta-expansion path should reject with the same ambiguity message the applied form uses, at the mention's position.
...
- [ ] `P3` **Records are invisible to CPR: `build_adt_ctor_index_go` indexes `ast.TypeDecl` only** (filed 2026-08-21, during the `SourcePos` ADT→record conversion). `build_adt_ctor_index_go` (`stdlib/compiler/ast_to_ir.sprout` ~7456) matches `ast.TypeDecl` and drops `ast.RecordDecl` into its `| _ ->` skip, so a record type gets **no `adt_index` entry**. Everything keyed off that index therefore treats a record result as un-unboxable: `cpr_result_lowerable` (~7095) is `adt_ctors_of_type(t) /= Nil`, so it returns `false`, so `build_fn_arities` marks the fn CPR-ineligible and `keep_cpr_eligible` filters it out of the worker set. A function returning a record is thus never given an unboxed worker, however scalar its fields.
  **Costs nothing today, and that is measured, not assumed.** The conversion's own hot path (`source.advance_position`, one `SourcePos` per codepoint lexed) emitted no `_worker` either *before* or *after* the change — CPR was not firing on the 3-scalar single-ctor ADT to begin with, so nothing regressed. This is latent, not live.
  **Why it is worth filing anyway:** the asymmetry is invisible at the call site and silently *caps* an optimization rather than breaking anything, which is the failure mode that survives review. It becomes live the moment CPR is extended — note `docs/scalar-replacement-v0.md` Stage 1 already had to special-case tuple results for exactly this reason (`adt_ctors_of_type` of a `TTuple` is `Nil`, and `translate_tail_catchall` errors on an empty ctor list), so "the result type has no ADT ctors" has now bitten twice from two different type forms. A record is a single-constructor product with a real tag and arity in the *same* `CtorInfo` table (`build_ctor_table_acc` ~551) — i.e. structurally the easiest possible CPR target — so the fix is plausibly just indexing `RecordDecl` alongside `TypeDecl` with its single `(tag, arity)` entry. Verify against the same tuple-shaped catch-all hazard before assuming that is sufficient, and gate on `just ir-golden-diff` (`tests/smoke_shapes/11_compiler_bundle.spr` is the golden that would show it).

- [ ] `P3` **String templates lower to more allocations than the `++` chain they replace; make the choice moot** (filed 2026-08-21). **Full analysis: [docs/string-building-v0.md](./docs/string-building-v0.md) §10** — that doc is the authority; this entry exists so the work is findable and is not a second copy of it.
  **The finding.** A backtick template lowers to *build a `List String` of the parts, then `string_concat_many`* (`desugar_ctx.desugar_template_general`), so it allocates **n+2** objects against `++`'s **n−1** — three more, at every size, with no part count at which it catches up. It buys back copying (linear vs `++`'s quadratic), so it loses below a ~1–2 KB result and wins above ~3 KB, reaching 9.41× at 13 KB. Measured 15-cell grid via `just bench-string-concat` (`scripts/bench_string_concat.sh`), reproducible.
  **Counter-intuitive part, worth not re-deriving:** the discriminator is bytes copied, **not part count**. `++` wins from 2 to 16 parts at 4-byte parts and only reaches a coin flip at 32, because each added part costs the template a `Cons` cell *and* a `to_string` call — so count loads both sides and cancels.
  **Why it is filed rather than fixed.** Not a measured bottleneck, so `AGENTS.md` §"Builtin vs Stdlib" rule 6 says speed alone does not justify touching the compiler. The real payoff is deleting the guidance burden: if small templates lowered to `++`, §§3–7 of that doc stop being something contributors must know, "choose on readability" becomes the whole rule, and the PR #171 mistake (opened believing interpolation *saved* allocations — it costs them) becomes impossible rather than merely documented.
  **Options, cheapest first** (detail in §10.2–10.3): (a) syntactic — emit a `++` chain from `desugar_template_general` when effective parts ≤ 4; measured to win-or-tie in every cell at 2 and 4 parts, one function, no types, no runtime change, but has a tail (four ~10 KB interpolations pessimised ~2.25×); (b) type-directed — an `ast_to_ir` peephole on the `string_concat_many(<cons-list>)` shape that picks `++` when every element's type has a compile-time-bounded render length (`Int`/`Bool`/`Char`/literal), which is tail-free and is the recommended variant; (c) runtime — a flat `alloca` buffer plus a `string_concat_n(ptr, count)` builtin, which removes the allocation deficit entirely and would delete the crossover, **but needs builtin approval up front** and careful shadow-stack rooting.
  **Blockers before code:** a prior-art survey of how C#/Java/Kotlin/Scala choose an interpolation lowering, verified against language references (not yet done — `AGENTS.md` §"Design Change Process" 3); builtin approval for (c). **And the regression test must assert on emitted IR**, not on the resulting string: the whole point is *which lowering is chosen*, and an output-only test passes under either, so it would let a silent revert through.
  **Already done, do not redo:** `desugar_ctx.sprout:14-17` special-cases empty → `""`, all-literal → one merged literal (a real constant-fold), and single-part → the part itself. Only the general case builds the list.
