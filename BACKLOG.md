...
- [ ] `P2` **A lambda whose parameter type is fixed only by something LATER gets its dictionary resolved too early, and resolved wrong (found 2026-08-20 while fixing the item above).** `fn get_labeller() -> Int -> String = \n -> label(n)` and `let f = \m -> label(m) in f(n)` — for any `where`-constrained `label`, **in a caller with no `where` clause of its own** — are rejected with ``dispatch-verify: dictionary for `ToString a` resolved to `ToString String` but the call fixes a = Int``. **This is not about constrained functions in value position, and it predates that work:** both shapes fail identically, written out by hand, on a compiler without the fix above. The tests for that item deliberately exclude these two positions for this reason.
  ...
  **Two directions, not chosen yet.** (a) Extend expected-type propagation to the two non-argument positions that actually have a declared type to propagate — a function's declared return type, and an annotated `let`. This is the fix that matches what a reader expects an annotation to do. (b) Defer dictionary resolution for a call whose constraint variable is still unresolved, so the choice happens after unification. Broader, and it touches the resolve/lowering split `M3b-5` deliberately settled — worth a design note before attempting.
  **The diagnostic is a separate, smaller defect either way.** The message describes a dictionary the user never wrote, in terms of a type variable from a signature they may not have looked at. After the fix above, `let f = label` reports at the right line with text that explains nothing actionable. A message naming the real problem — a lambda parameter whose type is not determined where it is needed — is worth having regardless of which direction lands.
  **One shape reports an INTERNAL ERROR rather than a diagnostic, and should be fixed before the rest.** An annotated top-level binding — `let labeller: Int -> String = label` — emits ``internal error: under-application of 'label' reached codegen (arity 2, got 1); the n-ary arity check should have rejected it before lowering``, plus the same text as an alloc-summary pre-pass warning. **Not a regression of working code:** on `origin/master` the identical file compiles and then dies at run time with the arity abort, i.e. it is the same P1 defect one stage earlier. But an internal error is the wrong *kind* of loud — it asks the user to report a compiler bug for ordinary source — and it does not meet the "located compile error" floor the item above sets. The annotation is the tell: a declared type is present and simply is not propagated to the initializer, which makes this the cheapest concrete instance of direction (a).
