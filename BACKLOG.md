...
- [ ] `P3` Deferred non-goals from `docs/filterable-v0.md` §7. (a) **`filter_map` over `Filterable`** — `Vec` has `vec_filter_map`, `List` has no equivalent at all, so the generic form and the missing List form should land together; PureScript/witherable both make `filterMap` the primitive and derive `filter` from it, which is the shape to weigh. (b) **`partition`** — no prelude form for either container. (c) **`Dict`/`Set` instances**, gated on those types getting `Functor`/`Foldable` first. (d) An **effectful predicate** (`a -> Bool !{e}`), which is the same B2 blocker as `class Each`.
