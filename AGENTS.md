...
**2-step bootstrap** (required when the committed seed predates a parser change):
`just refresh-seed` calls `bootstrap-from-seed` first, which rebuilds the binary from the committed seed. If that seed predates a syntax change, the rebuilt binary cannot parse the current source — a catch-22. Resolve it in two steps:
1. Temporarily revert files that *use* the new syntax (not the parser itself) to old syntax on disk, e.g. `git checkout master -- stdlib/compiler/compile_driver.sprout stdlib/compiler/codegen.sprout stdlib/compiler/lowering.sprout stdlib/prelude.sprout`.
2. Emit intermediate IR with the current binary: `./build/compile_driver_bin_stage1 --emit-ir stdlib stdlib/compiler/compile_driver.sprout > /tmp/intermediate.ll`. Verify output starts with `; Generated by sprout codegen.sprout` (old binary writes errors to stdout with exit 0). Copy to `bootstrap/compile_driver.ll`.
3. Restore HEAD files: `git checkout HEAD -- <files reverted in step 1>`.
4. Run `just refresh-seed` (now succeeds because the seed has the new parser baked in).
The intermediate binary has the new parser because `parser.sprout` — written in normal syntax — was in the dependency graph during step 2.
